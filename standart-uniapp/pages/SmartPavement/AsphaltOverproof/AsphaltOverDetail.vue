<template>
	<view id="waterOverDetail">
		...
		<view class="pourlist">
			<view class="pourlist-item">
				...
				<view class="pourlist-item-con">
					...
					<view class="pourlist-item-con-name">
						油石比：<span>{{ loaddata.youshibi }}</span>
					</view>
					<view class="pourlist-item-con-name">
						理论油石比：<span>{{ loaddata.llysb }}</span>
					</view>
					<view class="pourlist-item-con-name">
						出料温度(/℃)：<span>{{ parseInt(loaddata.chuliaowd) }}</span>
					</view>
					<view class="pourlist-item-con-name">
						总产量(/kg)：<span>{{ parseInt(loaddata.zongchanliang) }}</span>
					</view>
		...
		<view class="cldetail">
			...
			<uni-table stripe emptyText="暂无更多数据">
				<uni-tr>
					...
					<uni-th width="80" align="center">理论用量</uni-th>
					<uni-th width="80" align="center">误差值</uni-th>
					<uni-th width="80" align="center">超标值</uni-th>
				...
				<uni-tr v-for="(item, index) in loaddetail" :key="index">
					...
					<uni-td align="center"><text
							:class="[item.chaobiaodengji == 1?'elementary':item.chaobiaodengji == 2?'intermediate':item.chaobiaodengji == 3?'senior':'']">{{item.theoryNumber}}</text>
					</uni-td>
					<uni-td align="center"><text
							:class="[item.chaobiaodengji == 1?'elementary':item.chaobiaodengji == 2?'intermediate':item.chaobiaodengji == 3?'senior':'']">{{item.wucha}}</text>
					</uni-td>
					<uni-td align="center"><text
							:class="[item.chaobiaodengji == 1?'elementary':item.chaobiaodengji == 2?'intermediate':item.chaobiaodengji == 3?'senior':'']">{{item.chaobiao}}</text>
					</uni-td>
					<uni-td align="center">
		...
		<view v-if="loaddata.chaobiaodengji != 0 && loaddata.chaobiaodengji != null" class="btn">
			<u-button v-has="'AsphaltOverDetail:cz'"
			...
			<u-button v-has="'AsphaltOverDetail:sh'" v-show="loaddata.bhzCementOverHandler.overproofStatus === 10"
				class="btnclass" type="primary" @click="checkExa" plain text="监理审核"></u-button>
			<u-button v-has="'AsphaltOverDetail:sp'" v-show="loaddata.bhzCementOverHandler.overproofStatus === 40"
...
<script>
	export default {
		...
		onLoad(options) {
			this.loaddata = JSON.parse(options.ADetail)
			this.getloaddetail()
			// this.getmanprocess()
		},
		methods: {
			...
			checkMan() {
				uni.navigateTo({
					url: `/pages/smartbh/mix/handle/ChuZhi?batchNo=${this.loaddata.guid}&bhz=1&level=${this.loaddata.chaobiaodengji}`
				})
			},
			// 监理审核
			checkExa() {
				uni.navigateTo({
					url: `/pages/smartbh/mix/handle/ShenHe?batchNo=${this.loaddata.guid}&bhz=1&level=${this.loaddata.chaobiaodengji}`
				})
			},
			// 指挥部审批
			checkOver() {
				uni.navigateTo({
					url: `/pages/smartbh/mix/handle/ShenPi?batchNo=${this.loaddata.guid}&bhz=1&level=${this.loaddata.chaobiaodengji}`
				})
			},
			//详情
			checkDetail() {
