		...
		<view class="pourlist">
			<view v-for="(item,index) in listdata" :key="index" @click="checkDetail(item)" class="pourlist-item">
				...
				<view class="pourlist-item-con">
					...
					<view class="pourlist-item-con-name">
						总产量：<span>{{ parseInt(item.zongchanliang) }}</span>
					</view>
					<view class="pourlist-item-con-name">
						配方号：<span>{{ item.formulaNo }}</span>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			checkDetail(e) {
				uni.navigateTo({
					url: '/pages/SmartPavement/WaterOverproof/WaterOverDetail?wDetail=' + JSON.stringify(e)
				})
			},
			// 获取拌合站数据
			getloadlist() {
				...
				this.$http.get('/swbhz/bhzSwBases/czshList', {
					params
				}).then(res => {
					if (res.data.success) {
