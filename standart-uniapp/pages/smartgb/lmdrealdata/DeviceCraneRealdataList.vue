  ...
  <view class="wrap">
    ...
    <view class="wrap-section">
      ...
      <view class="wrap-section-box" v-for="(item, index) in Materialdata" :key="index">
        <navigator
          :url="
            '/pages/smartgb/lmdrealdata/DeviceCraneRealdataDetail?item=' +
            JSON.stringify(item)
          "
        >
          ...
          <view class="wrap-section-box-name"
            >设备名称:<span>{{ item.deviceCode_dictText }}</span>
          </view>
          <view class="wrap-section-box-name"
            >设备状态:<span>
              <!-- {{ changeStatus(item.cranedate) }} -->
              <view class="cu-tag bg-green" v-if="changeStatus(item.cranedate) === `在线`"
                >在线</view
              >
              <view class="cu-tag bg-grey" v-if="changeStatus(item.cranedate) === `离线`"
                >离线</view
              >
            </span></view
          >
          <view class="wrap-section-box-name"
            >主钩吊重(t):<span>{{ item.mainHookload }}</span></view
          >
          <view class="wrap-section-box-name"
            >预留副钩吊重(t):<span>{{ item.reservedVicehookload }}</span></view
          >
          <view class="wrap-section-box-name"
            >主钩载荷状态:<span>
              <view class="cu-tag bg-red" v-if="item.mainHookstatus === 1">预警</view>
              <view class="cu-tag bg-green" v-if="item.mainHookstatus === 0">正常</view>
            </span></view
          >
        </navigator>
      </view>
...
export default {
  ...
  methods: {
    ...
    Materiallist(e) {
      ...
      // this.Materialdata = []
      this.$http
        .get(`/devicecranerealdata/deviceCraneRealdata/list`, {
          params: {
            column: "id",
            order: "desc",
            pageNo: this.pageNo,
