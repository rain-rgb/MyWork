  ...
  <view class="wrap">
    ...
    <view class="wrap-section">
      ...
      <view class="wrap-section-box" v-for="(item, index) in Materialdata" :key="index">
        <navigator
          :url="'/pages/smartgb/bridgerealdata/DeviceCraneRealdataDetail?item=' + JSON.stringify(item)"
        >
          ...
          <view class="wrap-section-box-name"
            >设备名称:<span>{{ item.deviceCode_dictText }}</span></view
          >
          <view class="wrap-section-box-name"
            >设备状态:<span>
              <!-- {{ changeStatus(item.bridgedate) }} -->
              <view class="cu-tag bg-green" v-if="changeStatus(item.bridgedate) === `在线`">在线</view>
              <view class="cu-tag bg-grey" v-if="changeStatus(item.bridgedate) === `离线`">离线</view>
              </span>
          </view>
          <view class="wrap-section-box-name"
            >大车横向行程(m):<span>{{ item.bigTransverseCarroute }}</span></view
          >
          <view class="wrap-section-box-name"
            >大车纵向行程(m):<span>{{ item.bigLongitudinalCarroute }}</span></view
          >
          <view class="wrap-section-box-name"
            >1#天车吊钩载荷状态:<span>
              <view class="cu-tag bg-green" v-if="item.hookstatus1 === 0">载荷正常</view>
              <view class="cu-tag bg-red" v-if="item.hookstatus1 === 1">载荷预警</view>
            </span>
          </view>
          <view class="wrap-section-box-name"
            >2#天车吊钩载荷状态:<span>
              <view class="cu-tag bg-green" v-if="item.hookstatus2 === 0">载荷正常</view>
              <view class="cu-tag bg-red" v-if="item.hookstatus2 === 1">载荷预警</view>
            </span>
          </view>
        </navigator>
      </view>
...
import { checkTimeDifference } from "@/common/util/tool.js";
export default {
  components: {
    eq,
    MxDatePicker,
  ...
  methods: {
    changeStatus(e){
      let status = checkTimeDifference(e,5);
      let statusVal = "离线";
      if (status) {
        statusVal = "离线";
      }else{
    ...
    Materiallist(e) {
      ...
      // this.Materialdata = []
      this.$http
        .get(`/jiaqiaoji/deviceBridgeRealdata/list`, {
          params: {
            column: "id",
            order: "desc",
            pageNo: this.pageNo,
            PageSize: 10,
            deviceCode: this.Number,
    ...
    gystype() {
      this.$http
        .get(`/wzgongyingshang/wzgongyingshang/list`, {
          params: {
            sys_depart_orgcode: this.orgcode,
          },
        })
        .then((res) => {
          // console.log(res.data.result.records, "供应商信息");
          let data = res.data.result.records;
          this.gys = [];
          data.forEach((gysdata) => {
            this.gys.push(gysdata.gongyingshangname);
            this.gysid.push(gysdata.guid);
          });
        });
    },
    Pickgys(e) {
    ...
    clmtype() {
      this.$http.get(`/wzcailiaonamedict/wzcailiaonamedict/list`).then((res) => {
        // console.log(res.data.result.records, "材料名");
        let data = res.data.result.records;
        this.maters = [];
        this.Specifications = [];
        data.forEach((gysdata) => {
          this.maters.push(gysdata.cailiaono);
          this.Specifications.push(gysdata.cailiaoname);
          // console.log(this.maters, this.Specifications);
        });
      });
    },
