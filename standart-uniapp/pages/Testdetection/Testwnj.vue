	...
	<view class="wrap">
		...
		<view v-if="listdatavalue.length != 0">
			<view class="card" v-for="(item,index) in listdatavalue" :key="index">
				<!-- 路面跳转 -->
				<navigator :url="'/pages/Testdetection/Testwnjdetails?item='+JSON.stringify(item)">
					<view class="" v-if="item!==null?item:'暂无数据'">
						...
						<view>判定结果:<span
								:style="item.pdjg =='合格'?'color:green;font-weight:bold':item.pdjg =='不合格'?'color:red;font-weight:bold':'color:orange;font-weight:bold'">{{item.pdjg!==null?item.pdjg:'暂无数据'}}</span>
						</view>
		...
		<view class="screen" v-if="show" @confirm="confirm">
			<view class="screen-modal">
				...
				<view class="screen-modal-item">
					...
					<dict  dictCode="SYLX" @choose="choose"  title="请选择试验类型">   </dict>
...
<script>
	...
	export default {
		...
		methods: {
			...
			listdata() {
				this.$http.get("/syj/tSyjzb/list7", {
					params: {
						column: 'id',
						order: 'desc',
						sylx:this.sylxvalue,
