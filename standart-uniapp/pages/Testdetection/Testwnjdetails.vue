<template>
	<view>
		<cu-custom bgColor="bg-white" :isBack="true">
			<block slot="content">万能机详情</block>
		</cu-custom>
		<view class="card">
...
<script>
	...
	export default {
		...
		methods: {
			listData() {
				var that = this;
				this.$http.get("/syj/tSyjzb/selectwnjById", {
					// api.samplingList({
					params: {
				...
				this.$http.get("/syj/tSyjzb/selectwnjById", {
					...
					if (res.data.result.length > 0) {
						...
						dataList.forEach(res => {
							...
							this.kyvalue = res.lzgc.split(",");
							if (res.sjxh == 1) {
								this.kyvalue.forEach(res => {
									this.chartData.categories.push(` `) // x轴
								...
								this.kydata.forEach(res => {
									this.chartData1.categories.push(` `) // x轴
								...
								this.kydata.forEach(res => {
									this.chartData2.categories.push(` `) // x轴
								...
								this.kydata.forEach(res => {
									this.chartData3.categories.push(` `) // x轴
								...
								this.kydata.forEach(res => {
									this.chartData4.categories.push(` `) // x轴
								...
								this.kydata.forEach(res => {
									this.chartData.categories5.push(` `) // x轴
