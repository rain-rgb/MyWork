<template>
	<view class="wrap">
		<cu-custom bgColor="bg-white" :isBack="true">
			<block slot="content">张拉任务单新增</block>
		</cu-custom>
		<view class="nav">
			<view class="wrap-nav">
			...
			<view class="wrap-nav">
				<view class="screen-modal-item-name">张拉日期:</view>
				<view class="screen-picker">
					<view @click="timeShow = true">
						<u--input placeholder="请选择一次张拉日期" border="surround" v-model="timeValue" disabled
							suffixIcon="arrow-down"></u--input>
					</view>
				</view>
			</view>
			<view class="wrap-nav">
				<view class="screen-modal-item-name">张拉力:</view>
				<view class="screen-picker">
					<u--input placeholder="请输入一次张拉力" border="surround" v-model="onceTension"
						style="background-color: #F6F9FC;">
					</u--input>
				</view>
			</view>
			<view class="wrap-nav">
				<view class="screen-modal-item-name">设计强度:</view>
				<view class="screen-picker">
					<dict dictCode="betlev" title="请选择设计强度" @choose="durationBetlev">
					</dict>
				</view>
			</view>
			<view class="wrap-nav">
				<view class="screen-modal-item-name">误差指标:</view>
				<view class="screen-picker">
					<u--input placeholder="请输入两端千斤顶的张拉力误差指标" border="surround" v-model="deviation"
						style="background-color: #F6F9FC;">
					</u--input>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			addTask() {
				...
				let params = {
					shebeibianh: this.equipmentName,
					sgbwuuid: this.treeId,
					sgbwname: this.construction,
					projectname: this.projectName,
					zldate: this.timeValue,
					sjzll: this.onceTension,
					hntsjqd: this.strength,
					zllwc: this.deviation
				}
				this.$http.post('/sys/sysDepartproject/Zlrwdadd', params).then(res => {
