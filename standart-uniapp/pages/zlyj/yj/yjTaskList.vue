<template>
	<view id="pourmangage">
		<cu-custom bgColor="bg-white" :isBack="true">
			<block slot="content">压浆任务单</block>
			...
			<view v-for="(item,index) in listdata" :key="index" @click="checkDetail(item)"
				...
				<view class="pourlist-item-con">
					<view class="pourlist-item-con-name">
						设备名称：<span>{{ item.shebeibianhao_dictText }}</span>
					</view>
					<view class="pourlist-item-con-name">
						任务单号：<span>{{ item.uuid }}</span>
					</view>
					<view class="pourlist-item-con-name">
						施工部位：<span>{{ item.sgbwname }}</span>
					</view>
					<view class="pourlist-item-con-name">
						压浆任务状态：<span>
							<span style="color:red" v-if="item.status == '0'">未完成</span>
							<span style="color:green" v-if="item.status == '1'">已完成</span>
						</span>
					</view>
					<view class="pourlist-item-con-name">
