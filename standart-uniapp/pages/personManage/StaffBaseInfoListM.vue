	...
	<view id="home">
		...
		<view>
			<view class="section" v-for="(item,index) in shitilist" :key="index">
				...
				<view class="">标段:<span>{{item.biaoduan !==null?item.biaoduan:'暂无数据'}}</span></view>
				<view class="">姓名:<span>{{item.xingming !==null?item.xingming:'暂无数据'}}</span></view>
				<view class="">电话:<span>{{item.dianhua !==null?item.dianhua:'暂无数据'}}</span></view>
				<view class="">建设处:<span>{{item.jianshechu !==null?item.jianshechu:'暂无数据'}}</span></view>
				<view class="">法人单位:<span>{{item.farendanwei !==null?item.farendanwei:'暂无数据'}}</span></view>
				<view class="">人员类别:<span>{{item.renyuantype !==null?item.renyuantype:'暂无数据'}}</span></view>
				<view class="">职务:
				  <span>
				    {{ item.zhiwu || item.banzu || '' }}
				  </span>
				</view>
		...
		<view class="screen" v-if="show">
		  <view class="screen-modal">
		    ...
		    <!-- 标段筛选 -->
		    <view class="screen-modal-item">
		      <view class="screen-modal-item-name">标段：</view>
		      <u--input 
		        placeholder="请输入标段名称" 
		        v-model="biaoduan"  
		      ></u--input>
	...
	export default {
		...
		methods: {
			...
			textlist() {
				...
				this.$http.get(`/staffbase/staffBaseInfo/list`, {
					...
					params: {
						...
						biaoduan: this.biaoduan,  // 直接传递输入值
						pageNo: this.pageNo,
						pageSize: 10,
