		...
		<view>
			<view class="section" v-for="(item,index) in shitilist" :key="index">
				<view class="card" style="position: relative;">
					...
					<!-- </navigator> -->
					<u-button @click="checkConfirm(item.danhao,item.shrwdh)" v-has="'cjqr:confirm'"
						style="width:90px;border-radius: 10px;color: #fff;background-color: #19be6b; position: absolute;bottom: 56px;right: 30px;"
						text="抽检确认"></u-button>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//抽检确认
			checkConfirm(danhao, shrwdh) {
				let params = {
					fcid: danhao,
					cailiaono: shrwdh
				}
				this.$http.post(`/syfccj/syFccj/add`, params).then(res => {
					// console.log(res, "抽检确认")
					if (res.data.code == 200) {
						uni.showToast({
