<template>
  <view id="mixpile">
    <cu-custom bgColor="bg-white" :isBack="true">
      <block slot="content">灌注桩信息</block>
      ...
      >
        <view class="list-item-con">
          ...
          <view class="list-item-con-name">
            状态：
            <!-- <span
              :style="{ color: item.chaobiaodengji == 3 ? '#EA0808' : '#6DD400' }"
            >
              {{
                isLessThanMinutesAgo(item.chaobiaodengji)
                  ? "离线"
                  : item.worksta == 0
                  ? "在线"
                  : "下钻"
          ...
          <view class="list-item-con-name">
            密度：<span>{{ parseFloat(item.pileDensity).toFixed(2) }} kg/m³</span>
          </view>
...
export default {
  ...
  methods: {
    checkDetail(e) {
      // if (this.isLessThanMinutesAgo(e.updateTime)) {
      //   uni.showToast({
      //     title: "此灌注桩不在线，请选择其他设备！！",
      //     icon: "none",
      //   });
      // } else {
        let queryString = encodeURIComponent(JSON.stringify(e)); // 编码字符串以处理特殊字符
        uni.navigateTo({
          url: "/pages/ruanji/fillingPileDetail?shebeino=" + e.sbbh + "&id=" + e.id,
        });
      // }
    },
    ...
    getDeviceOpen() {
      ...
      this.$http
        .get("devicemixgrouted/deviceMixpileGroutedReal/list", {
          params,
        })
        .then((res) => {
