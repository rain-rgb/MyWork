<template>
	<view id="pilerecorddetail">
		<cu-custom bgColor="bg-white" :isBack="true">
			<block slot="content">桩记录详情</block>
		</cu-custom>
		<view class="list">
			<view class="list-item" v-if="listdata.deviceMixpileHistorydataOneList.length > 0">
				<view class="list-item-con">
					...
					<!-- <view class="list-item-con-name">
						水泥用量：<span>{{ parseFloat(listdata.pileCement).toFixed(3) }}</span>
					</view> -->
					<view class="list-item-con-name">
						实际深度：<span>{{ parseFloat(listdata.pileRealdep).toFixed(3) }}</span>
					</view>
					<view v-if="listdata.chaobiaodengji != 0" class="list-item-con-name">
						预警原因：<span>{{ listdata.problem }}</span>
					</view>
					...
					<view class="list-item-con-name">
						下钻浆量(方)：<span>{{ parseFloat(listdata.pileDownbeton).toFixed(3) }}</span>
					</view>
					<view class="list-item-con-name">
					...
					<view class="list-item-con-name">
						提钻浆量(方)：<span>{{ parseFloat(listdata.pileUobeton).toFixed(3) }}</span>
					</view>
					<view class="list-item-con-name">
						注浆量(方)：<span>{{ (parseFloat(listdata.pileUobeton) + parseFloat(listdata.pileDownbeton)).toFixed(3) }}</span>
					</view>
					<view class="list-item-con-name">
		...
		<view class="main">
			...
			<view class="main-item">
				<view style="width: 33%;" v-if="listdata.deviceMixpileHistorydataOneList.length > 0">
					<uni-table>
						<uni-tr>
							<uni-th>搅拌次数</uni-th>
						</uni-tr>
						<uni-tr v-for="(item,index) in nameList" :key="index">
							<uni-th>
								<u--input placeholder="/" border="none" v-model="item.name" disabled disabledColor="#fff">
								</u--input>
							</uni-th>
						</uni-tr>
					</uni-table>
				...
				<view style="width: 67%;">
					<view v-if="listdata.deviceMixpileHistorydataOneList.length > 0">
						<scroll-view scroll-x="true">
							<view :style="`width: ${240*(listdata.deviceMixpileHistorydataOneList.length <= 2 ? 2:listdata.deviceMixpileHistorydataOneList)}upx;`">
							<!-- <view :style="`width: 480upx;`"> -->
								<uni-table>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											{{ item.gzcount === 1 ? '初搅' : '复搅' }}
										</uni-th>
									</uni-tr>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											<u--input placeholder="/" border="none" v-model="item.pileRealdep"  disabled
												disabledColor="#fff">
											</u--input>
										</uni-th>
									</uni-tr>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											<u--input placeholder="/" border="none" v-model="item.pileWorktime"  disabled
												disabledColor="#fff">
											</u--input>
										</uni-th>
									</uni-tr>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											<u--input placeholder="/" border="none" v-model="item.pileDspeed"  disabled
												disabledColor="#fff">
											</u--input>
										</uni-th>
									</uni-tr>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											<u--input placeholder="/" border="none" v-model="item.pileDelectr"  disabled
												disabledColor="#fff">
											</u--input>
										</uni-th>
									</uni-tr>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											<u--input placeholder="/" border="none" v-model="item.pileUspeed"  disabled
												disabledColor="#fff">
											</u--input>
										</uni-th>
									</uni-tr>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											<u--input placeholder="/" border="none" v-model="item.pileUelectr"  disabled
												disabledColor="#fff">
											</u--input>
										</uni-th>
									</uni-tr>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											<u--input placeholder="/" border="none" v-model="item.pileDownbeton"  disabled
												disabledColor="#fff">
											</u--input>
										</uni-th>
									</uni-tr>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											<u--input placeholder="/" border="none" v-model="item.pileMinelec"  disabled
												disabledColor="#fff">
											</u--input>
										</uni-th>
									</uni-tr>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											<u--input placeholder="/" border="none" v-model="item.pileDensity"  disabled
												disabledColor="#fff">
											</u--input>
										</uni-th>
									</uni-tr>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											<u--input placeholder="/" border="none" v-model="item.pileCement"  disabled
												disabledColor="#fff">
											</u--input>
										</uni-th>
									</uni-tr>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											<u--input placeholder="/" border="none" v-model="item.pileMaxelectr"  disabled
												disabledColor="#fff">
											</u--input>
										</uni-th>
									</uni-tr>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											<u--input placeholder="/" border="none" v-model="item.pileRatio"  disabled
												disabledColor="#fff">
											</u--input>
										</uni-th>
									</uni-tr>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											<u--input placeholder="/" border="none" v-model="item.pileX"  disabled
												disabledColor="#fff">
											</u--input>
										</uni-th>
									</uni-tr>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											<u--input placeholder="/" border="none" v-model="item.pileStarttime"  disabled
												disabledColor="#fff">
											</u--input>
										</uni-th>
									</uni-tr>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											<u--input placeholder="/" border="none" v-model="item.pileTime"  disabled
												disabledColor="#fff">
											</u--input>
										</uni-th>
									</uni-tr>
									<uni-tr>
										<uni-th v-for="(item,index) in listdata.deviceMixpileHistorydataOneList" :key="index">
											<u--input placeholder="/" border="none" v-model="item.chaobiaodengji"  disabled
												disabledColor="#fff">
											</u--input>
										</uni-th>
									</uni-tr>
		...
		<view class="poursta">
			...
			<view class="poursta-pic">
				<view class="charts-box1" v-if="this.quxiandata.categories.length > 0">
		...
		<view class="poursta">
			...
			<view class="poursta-pic">
				<view class="charts-box1" v-if="this.chartData2.categories.length > 0">
	...
	export default {
		...
		methods: {
			...
			getloadlist() {
				...
				ruanjiapi.pilenotelist({
					params
				}).then(res => {
					this.listdata = res.data.result.records[0];
					this.listdata.deviceMixpileHistorydataOneList.forEach((e)=>{
						e.pileRealdep = e.pileRealdep.substring(0, this.listdata.pileRealdep.indexOf('.') + 3);
						e.pileDspeed = Number(e.pileDspeed).toFixed(3);
						e.pileDelectr = Number(e.pileDelectr).toFixed(3);
						e.pileUspeed = Number(e.pileUspeed).toFixed(3);
						e.pileUelectr = Number(e.pileUelectr).toFixed(3);
						e.pileDownbeton = (Number(e.pileDownbeton) + Number(e.pileUobeton)).toFixed(3);
						e.pileDensity = Number(e.pileDensity).toFixed(3)
						e.pileCement = Number(e.pileCement).toFixed(3);
						e.pileMaxelectr = Number(e.pileMaxelectr).toFixed(3);
						e.pileRatio = e.pileRatio.substring(0, this.listdata.pileRatio.indexOf('.') + 3);
						e.pileX = e.pileX.substring(0, this.listdata.pileX.indexOf('.') + 3);
						e.chaobiaodengji = (e.chaobiaodengji === 0 ? '合格' : '不合格');
					})
					// for (let index = 0; index < this.listdata.deviceMixpileHistorydataOneList.length; index++) {
					// 	const e = this.listdata.deviceMixpileHistorydataOneList[index];
					// 	e.pileRealdep = e.pileRealdep.substring(0, this.listdata.pileRealdep.indexOf('.') + 3);
					// 	e.pileDspeed = Number(e.pileDspeed).toFixed(3);
					// 	e.pileDelectr = Number(e.pileDelectr).toFixed(3);
					// 	e.pileUspeed = Number(e.pileUspeed).toFixed(3);
					// 	e.pileUelectr = Number(e.pileUelectr).toFixed(3);
					// 	e.pileDownbeton = (Number(e.pileDownbeton) + Number(e.pileUobeton)).toFixed(3);
					// 	console.log(e.pileRealdep,index);
					// }
				}).catch(e => {
					console.log("请求错误", e)
				})
			...
			loadData() {
				...
				this.$http.get('/devicemixpilehistorydatapart/deviceMixpileHistorydataPart/list', {
					...
					let speed = [] //速度
					datas.forEach(e => {
						datatime.push(e.partEndtime.split(' ')[1])
						...
						curdep.push(parseFloat(e.partDep).toFixed(2))
						elec.push(parseFloat(e.partElectricity || 0).toFixed(2))
						flowlst.push(parseFloat(e.partBeton).toFixed(2))
						speed.push(parseFloat(e.partSpeed).toFixed(2))
					})
