<template>
	<view id="pipedata">
		<cu-custom bgColor="bg-white" :isBack="true">
			<block slot="content">预应力管桩预警信息</block>
			<view slot="right" @click="screen">筛选</view>
		</cu-custom>
		<u-loading-page :loading="loading" loading-text="加载中..."></u-loading-page>
		<view class="top">
			<view class="top-content">
				<view class="num yellow">{{ pileNum.yjzs || 0 }}</view>
				<view class="title">预警总数</view>
			</view>
			<view class="top-content">
				<view class="num red">{{ pileNum.wbh || 0 }}</view>
				<view class="title">未闭合</view>
			</view>
			<view class="top-content">
				<view class="num blue">{{ pileNum.ybh || 0 }}</view>
				<view class="title">已闭合</view>
			</view>
		</view>
		<view class="pourlist">
			<view v-for="(item,index) in listdata" :key="index" class="pourlist-item">
				<view class="pourlist-item-sta">
					<image v-if="item.chaobiaodengji === 0" src="../../static/pour/hege.png">
					</image>
					<image v-if="item.chaobiaodengji === 1" src="../../static/pour/yj.png">
					</image>
				</view>
				<view class="pourlist-item-sta1">
					<image v-if="item.overproofStatus==0" src="../../static/pour/handle4.png">
					</image>
					<image v-if="item.overproofStatus==10" src="../../static/pour/handle7.png">
					</image>
					<image v-if="item.overproofStatus==20" src="../../static/pour/handle1.png">
					</image>
				</view>
				<view class="pourlist-item-con">
					...
					<view class="pourlist-item-con-name">
						预警原因：<span>{{ item.ycyy }}</span>
					</view>
					<view class="pourlist-item-con-name">
						预警时间：<span>{{ item.datatime }}</span>
					</view>
	...
	export default {
		...
		methods: {
			...
			getloadlist() {
				...
				this.$http.get('/devicepipepilehistorydataone/devicePipepileHistorydataOne/cbcxlist', {
					params
				}).then(res => {
			...
			// 获取预警数
			getpilelist() {
				this.$http.get('/devicepipepilehistorydataone/devicePipepileHistorydataOne/cbbhlist').then(res => {
					if (res.data.success) {
						this.pileNum = res.data.result
					}
				})
			},
			// 获取设备列表
