...
<script>
	...
	export default {
		...
		created: function() {
			...
			console.log("进入画面登录画面111---------------" + this.$route.query.username);
			
			let a= decodeURIComponent(Object.values(this.$route.query)[0]) 
			// let a= Object.values(this.$route.query)[0] 
			console.log(a)
			let index = a.lastIndexOf("Bearer");
			this.userName = decodeURIComponent(a.substring(index, a.length -2));
		...
		onLoad: function() {
			...
			// console.log(tokenUrl, 'token----------------------',index)
			// this.userName = tokenUrl;
			this.userName = options.query.username
			this.onLogin()
			plus.runtime.getProperty(plus.runtime.appid, function(wgtinfo) {
				that.version = wgtinfo.version
			});
		...
		methods: {
			...mapActions(["mLoginss", "mLogin", "PhoneLogin", "ThirdLogin", "GetPermissionList", "Logout"]),
			onLogin: function() {
				...
				this.departList = []
				this.orgCodeList = []
				this.mLoginss(loginParams).then((res) => {
					this.loading = false;
					if (res.data.success) {
