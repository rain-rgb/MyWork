...
export default {
  ...
  methods: {
    getParams(options) {
      ...
      this.chepai = this.params.driver;
      this.begintime = this.params.begtim;
      // this.begintime = this.params.begtim.split(" ")[0] + " " + "00:00:00";
      this.endtime =
        this.params.qianshoutime == null
          ? this.getNowDateTime()
          : this.params.qianshoutime;
      this.markers[0].callout.content = `车牌号：${this.chepai}`;
      // console.log(this.endtime, "params ----------------------------");
			this.getSheBei();
    },
    getNowDateTime() {
      let date = new Date();
    ...
    getSheBei(){
      this.$http.get(`/car/schedulingCar/listyafcph`, {
      // this.$http.get(`/car/schedulingCar/list`, {
        params: {
          vehicle: this.params.vehicle,
          sys_depart_orgcode:this.params.sysOrgCode,
          // sbbh: this.sbNumber,
          // shebeibianhao: this.sbNumber,
        }
      }).then(res => {
        if (res.data.code == 200) {
          this.sbjno = res.data.result.sbjno
      this.Maplocation();
