 ...
 */
@Data
@ApiModel(value="bhz_cement_basePage对象", description="拌合站主表")
public class BhzCementBaseCarPage {
	...
	private List<Detail> detailcolumns;// 自动列名
	private List<Map> detailLists;// 生产详情

	private Double onellz;// 单盘理论值
	private Double onesjz;// 单盘实际值



	public void setBhzCementDetailList(List<BhzCementDetail> bhzCementDetailList) {
//		this.bhzCementDetailList = bhzCementDetailList;
		if(bhzCementDetailList.size()>0){
			List<Map> ds = new ArrayList<>();
			List<Detail> c = new ArrayList<>();
            HashMap llz = new HashMap<>();
			llz.put("para","理论值(kg)");
			HashMap sjz =new HashMap();
			sjz.put("para","实际值(kg)");
			HashMap wcz = new HashMap<>();
			wcz.put("para","误差值(%)");
			HashMap cbdj = new HashMap<>();
			cbdj.put("para","超标等级");
			 Double onellz1 = 0.0;// 单盘理论值
			 Double onesjz1 = 0.0;// 单盘实际值

			for (BhzCementDetail detail: bhzCementDetailList){
				Detail column = new Detail();
				column.setTitle(detail.getMaterialeName());
				column.setDataIndex(detail.getMaterialeName());
				c.add(column);
				llz.put(detail.getMaterialeName(),detail.getTheoryNumber());
				onellz1 = onellz1 +detail.getTheoryNumber();
				sjz.put(detail.getMaterialeName(),detail.getRealityNumber());
				onesjz1 = onesjz1 + detail.getRealityNumber();
				wcz.put(detail.getMaterialeName(),detail.getOverValue());
				cbdj.put(detail.getMaterialeName(),detail.getMaterialeOverLevel());
			}
			ds.add(llz);
			ds.add(sjz);
			ds.add(wcz);
			ds.add(cbdj);
			this.detailLists = ds;
			this.detailcolumns = c;
			this.onellz = Double.parseDouble(String.format("%.2f",onellz1));
			this.onesjz =Double.parseDouble(String.format("%.2f",onesjz1));
		}
	}

	public void setDetailLists(List<Map> detailLists) {
