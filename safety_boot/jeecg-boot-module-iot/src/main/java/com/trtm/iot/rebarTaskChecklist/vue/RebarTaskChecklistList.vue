<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="download" @click="handleExportXls('rebar_task_checklist')">导出</a-button>
    ...
    <rebar-task-checklist-modal ref="modalForm" @ok="modalFormOk"></rebar-task-checklist-modal>
  </a-card>
</template>
...
<script>
  ...
  import RebarTaskChecklistModal from './modules/RebarTaskChecklistModal'

  export default {
    name: 'RebarTaskChecklistList',
    mixins:[JeecgListMixin, mixinDevice],
    components: {
      RebarTaskChecklistModal
    },
    data () {
      return {
        description: 'rebar_task_checklist管理页面',
        // 表头
        columns: [
          {
            title: '#',
    ...
    methods: {
      ...
      getSuperFieldList(){
        ...
        fieldList.push({type:'string',value:'constructionSite',text:'施工部位'})
        fieldList.push({type:'string',value:'teamLeader',text:'班组长'})
        fieldList.push({type:'string',value:'applicant',text:'申请人'})
        fieldList.push({type:'string',value:'applicantUnit',text:'申请单位'})
        fieldList.push({type:'string',value:'orgCode',text:'组织机构编码'})
        fieldList.push({type:'string',value:'status',text:'状态'})
        fieldList.push({type:'date',value:'useTime',text:'使用时间'})
