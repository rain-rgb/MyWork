        ...
        <a-row>
          ...
          <a-col :span="24">
            <a-form-item label="提梁状态" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input-number v-decorator="['status']" placeholder="请输入提梁状态" style="width: 100%" />
            </a-form-item>
          </a-col>
          <a-col :span="24">
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          add: "/tiliang/tiliangProcedure/add",
          edit: "/tiliang/tiliangProcedure/edit",
          queryById: "/tiliang/tiliangProcedure/queryById"
        }
      }
    },
    computed: {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'uuid','status','endtime','createBy','createTime'))
        })
      },
      //渲染流程表单数据
      showFlowData(){
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'uuid','status','endtime','createBy','createTime'))
      },
    }
  }
</script>