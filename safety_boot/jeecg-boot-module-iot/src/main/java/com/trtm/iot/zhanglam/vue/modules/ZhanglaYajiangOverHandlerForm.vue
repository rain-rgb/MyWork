  ...
  <a-spin :spinning="confirmLoading">
    <j-form-container :disabled="formDisabled">
      <a-form-model ref="form" :model="model" :rules="validatorRules" slot="detail">
        <a-row>
          <a-col :span="24">
            <a-form-model-item label="对应张拉压浆主表的syjid" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="baseid">
              <a-input v-model="model.baseid" placeholder="请输入对应张拉压浆主表的syjid"  ></a-input>
            </a-form-model-item>
          </a-col>
          <a-col :span="24">
...
<script>
  ...
  export default {
    name: 'ZhanglaYajiangOverHandlerForm',
    components: {
    },
    props: {
      //表单禁用
    ...
    data () {
      return {
        ...
        validatorRules: {
           overproofStatus: [
              { required: true, message: '请输入超标状态：0为未处理，10为施工方已处理，20为监理方已处理!'},
           ],
        },
        url: {
          add: "/zhanglam/zhanglaYajiangOverHandler/add",
          edit: "/zhanglam/zhanglaYajiangOverHandler/edit",
          queryById: "/zhanglam/zhanglaYajiangOverHandler/queryById"
        }
      }
    },
    computed: {
