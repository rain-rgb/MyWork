        ...
        <a-row>
          <a-col :span="24">
            <a-form-item label="台座编号" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-decorator="['taizuono']" placeholder="请输入台座编号"  ></a-input>
            </a-form-item>
          </a-col>
          <a-col :span="24">
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          add: "/zhiliangtaizuocfg/zhiliangTaizuoCfg/add",
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'taizuono','taizuoname','sysOrgCode'))
        })
      },
      //渲染流程表单数据
      showFlowData(){
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'taizuono','taizuoname','sysOrgCode'))
      },
    }
  }
</script>