 ...
 */
@Data
@TableName("chaoshengbo_syqxsj")
@Accessors(chain = true)
@EqualsAndHashCode(callSuper = false)
@ApiModel(value="chaoshengbo_syqxsj对象", description="chaoshengbo_syqxsj")
public class ChaoshengboSyqxsj implements Serializable {
	...
	/**剖面缺陷数据1-5:以”,”分隔组成的数据, 每个剖面可以存储5 处缺陷值，由5个float类型数据组成*/
	@Excel(name = "剖面缺陷数据1-5:以”,”分隔组成的数据, 每个剖面可以存储5 处缺陷值，由5个float类型数据组成", width = 15)
    @ApiModelProperty(value = "剖面缺陷数据1-5:以”,”分隔组成的数据, 每个剖面可以存储5 处缺陷值，由5个float类型数据组成")
    private java.lang.String poumianquexian1;
	/**剖面缺陷数据1-5:以”,”分隔组成的数据, 每个剖面可以存储5 处缺陷值，由5个float类型数据组成*/
	@Excel(name = "剖面缺陷数据1-5:以”,”分隔组成的数据, 每个剖面可以存储5 处缺陷值，由5个float类型数据组成", width = 15)
    @ApiModelProperty(value = "剖面缺陷数据1-5:以”,”分隔组成的数据, 每个剖面可以存储5 处缺陷值，由5个float类型数据组成")
    private java.lang.String poumianquexian2;
	/**剖面缺陷数据1-5:以”,”分隔组成的数据, 每个剖面可以存储5 处缺陷值，由5个float类型数据组成*/
	@Excel(name = "剖面缺陷数据1-5:以”,”分隔组成的数据, 每个剖面可以存储5 处缺陷值，由5个float类型数据组成", width = 15)
    @ApiModelProperty(value = "剖面缺陷数据1-5:以”,”分隔组成的数据, 每个剖面可以存储5 处缺陷值，由5个float类型数据组成")
    private java.lang.String poumianquexian3;
	/**剖面缺陷数据1-5:以”,”分隔组成的数据, 每个剖面可以存储5 处缺陷值，由5个float类型数据组成*/
	@Excel(name = "剖面缺陷数据1-5:以”,”分隔组成的数据, 每个剖面可以存储5 处缺陷值，由5个float类型数据组成", width = 15)
    @ApiModelProperty(value = "剖面缺陷数据1-5:以”,”分隔组成的数据, 每个剖面可以存储5 处缺陷值，由5个float类型数据组成")
    private java.lang.String poumianquexian4;
	/**剖面缺陷数据1-5:以”,”分隔组成的数据, 每个剖面可以存储5 处缺陷值，由5个float类型数据组成*/
	@Excel(name = "剖面缺陷数据1-5:以”,”分隔组成的数据, 每个剖面可以存储5 处缺陷值，由5个float类型数据组成", width = 15)
    @ApiModelProperty(value = "剖面缺陷数据1-5:以”,”分隔组成的数据, 每个剖面可以存储5 处缺陷值，由5个float类型数据组成")
    private java.lang.String poumianquexian5;
}
