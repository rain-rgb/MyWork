...
//        signRequest.put("stampers", mapList);
    ...
    public static void sweepcode(String contractId) throws Exception {
        HttpParamers httpParamers = new HttpParamers(com.trtm.api.utils.http.HttpMethod.GET);
    ...
    public  String viewurl(String contractId) throws Exception {
        HttpParamers httpParamers = new HttpParamers(com.trtm.api.utils.http.HttpMethod.GET);
        httpParamers.addParam("contractId", contractId);
        httpParamers.addParam("pageType", "CONTENT");
//        httpParamers.addParam("viewUser.mobile", "15270292717");
        String signUrl = baseServiceUrl + "/contract/viewurl";
        // 设置 header 参数：每调用一次接口，都需要重新生成
        header = generateHttpHeader(appToken, appSecret);
        try {
            String response = HttpClient.doGet(signUrl, httpParamers, header, 15000, 60000);
    ...
    public  void main1(String[] args) throws Exception {
        String documentId = createbyfile("https://safety-boots.oss-cn-beijing.aliyuncs.com/upload/物联网指令单(ZLD-监理办JL01-20230628-002)_1688976565434.pdf");
...
//        createEmployee("高鑫玉","15957140410");
        ...
        Map<String, String> map1 = new HashMap<String, String>() {{
            put("operatorName", "温承星");
            put("operatorContact", "15270292717");
        }};
        ...
        String contractId = createbycategory(documentId,mapList); // 使用第一步生成的文档 ID 创建并发起电子签约，调用接口【创建或发起电子签约】，获取【电子签约】ID。
//        String contractId = "3115923983296373188";
//        signurl(contractId,"15270292717");
//        pageurl(contractId,"15270292717");
//        verify(documentId); // 获取个人扫码签链接
//        rolelist();
        companysignsilent(); // 单位印章静默签署授权链接
