...
public class gxmjob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.SLJBZ_XMGK);//水泥搅拌桩大屏-接口文档v1.3 各项目概况
        //如果他等于空
        ...
        String urling = "smwdata/projectPreview?";
        Date date = new Date();
        ...
        if (shebeiInfos.size() > 0){
            ...
            for (ShebeiInfo shebeiInfo :shebeiInfos){
                ...
                if (queryone != null){
                    ...
                    double v = Double.parseDouble(queryone.getWorklength());
                    i += pilecount;
                    j += v;
                }else {
                    log.info("该设备没有实时数据。");
            ...
            List<ProjectPreviewVo> list = new ArrayList<>();
            ProjectPreviewVo projectPreviewVo = new ProjectPreviewVo();
            projectPreviewVo.setProjectId(orgcode);//项目编码
            projectPreviewVo.setFinishPileNum(i);//已完成桩基数（根）
            projectPreviewVo.setProjectName(departname.getDepartNameAbbr());//项目名
            projectPreviewVo.setContractSection("");//合同段
            projectPreviewVo.setConstructionUnit("");//施工单位
            projectPreviewVo.setWayLeve("");//公路等级
            projectPreviewVo.setSupervisorUnit("");//监理单位
            projectPreviewVo.setFinishProjectLong(j);//已完完工程总量（米）
            projectPreviewVo.setDesignPileNum(null);//项目设计桩基数
            list.add(projectPreviewVo);
            String data = JSON.toJSONString(list, SerializerFeature.MapSortField);
    ...
    // url
    public static Boolean getCode(String urling,long time,String singe,String data) {
