...
public class ZhydJob  implements Job {
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.ZHYD_CFG);
        //如果他为空,日志，并直接return
...
//        Integer curid = selectsysconfigone.getCurid();
        List<DeviceElectricRealdata> selectzhydones = deviceElectricRealdataService.selectzhydone(0);
        if (null == selectzhydones || selectzhydones.size() == 0) {
            log.info(String.format("暂无智慧用电未判断的数据"));
            return;
        }
        int id = 0;
        for (DeviceElectricRealdata selectzhydone : selectzhydones) {
            id = selectzhydone.getId();
            ...
            String elecdate = DateUtil.format(selectzhydone.getElecdate(), "yyyy-MM-dd HH:mm:ss");//数据上传时间
            //设备编号
            String imei = selectzhydone.getImei();
            try {
...
//                        primaryNames = bhzPhone.getNames();
                ...
                Map map = zhydJobUntil.zhydOver(selectzhydone,selectshebeione,is_call);
                ...
                if (0 == final_over_level || final_content.length() == 0){
                    final_over_level = 1;
                    deviceElectricRealdataService.updateStatus(id,final_over_level);
                    deviceElectricRealdataService.updatezhydone(id,1);//修改device_electric_realdata的状态(是否进行超标判断)
                    continue;
                }
                ...
                DeviceElectricRealdata deviceElectricRealdata = new DeviceElectricRealdata();
                deviceElectricRealdata.setId(id);
                deviceElectricRealdata.setStatus(final_over_level);
                deviceElectricRealdata.setAlertstate(1);
                deviceElectricRealdataService.updateById(deviceElectricRealdata);  //智慧用电修改超标状态以及检测过状态
            }catch (Exception e){
                e.printStackTrace();
                deviceElectricRealdataService.updatezhydone(id,40);

            }
            log.info(String.format("智慧用电实时数据表超标检测！   时间" +DateUtils.now(),"当前判断到"+id));
        }
        //sysConfigService.updateSysConfig(JobUtil.ZHYD_CFG,id);
    }
}
