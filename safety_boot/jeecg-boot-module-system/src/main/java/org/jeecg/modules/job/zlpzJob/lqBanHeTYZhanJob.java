...
public class lqBanHeTYZhanJob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.ZLPZ_LQTY);//沥青拌合站
        //如果他等于空
        ...
        List<BhzLqBases> bhzLqBasess = bhzLqBasesService.selectListTY(shebeilist, curid);
        ...
        for (BhzLqBases bhzLqBases : bhzLqBasess) {
            ...
            Date chuliaoshijiandate = null;
            try {
                chuliaoshijiandate = sdf.parse(chuliaoshijian);
            } catch (ParseException e) {
                e.printStackTrace();
            }
            String formattedDate = sdf.format(chuliaoshijiandate);
            sendDate.set("colTime", formattedDate);
            int alarmStatus = 0;
            ...
            if (result == 0) {
                ...
                log.info(String.format("浙路品质沥青拌合站推送成功！%s", back));
            } else {
                bhzLqBases.setIszlpz(2);
                log.info(String.format("浙路品质沥青拌合站推送失败！%s%s", sendJsonObject, back));
            }
