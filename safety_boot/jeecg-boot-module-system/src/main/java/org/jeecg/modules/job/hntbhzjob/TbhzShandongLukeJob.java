 ...
 * \* Date: 2021/6/2
 * \* Time: 11:23
 * \* Description:山东陆科推送砼拌合站实时数据(实时数据包含所有的合格不合格数据 注：推送的数据已经判断过超不超标)
 * \
 */
@Slf4j
@PersistJobDataAfterExecution
@DisallowConcurrentExecution
public class TbhzShandongLukeJob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.HNTBHZ_TUISONG_SHANDONGLUKE);//砼拌合站数据推送山东陆科
        //如果他等于空
        if (null == selectsysconfigone) {
            log.info(String.format("未获取到砼拌合站推送山东陆科定时任务的配置信息" + DateUtils.now()));
            return;
        }
        //当前数据检测到的目标id
        ...
        String[] split1 = sbjnolist.split(",");
        List<String> strsToList1= Arrays.asList(split1);
        List<BhzCementBase> selecthntbhzones = bhzCementBaseService.selecthntbhzList(curid, 1,strsToList1);
        ...
        for (BhzCementBase selecthntbhzone : selecthntbhzones) {
            ...
            if(bhzCementDetails.size()>0){
                ...
                for (BhzCementDetail bhzCementDetail : bhzCementDetails) {
                    if(bhzCementDetail.getMaterialeType()==1){//细骨料
                        if(datelist[1]==0){
                            publicMixingStationList1.putOpt("thinOne",bhzCementDetail.getRealityNumber());//细骨料1
                            publicMixingStationList1.putOpt("thinOneKg",bhzCementDetail.getTheoryNumber());//细骨料1配比
                            datelist[1]=1;
                        }else if(datelist[1]==1){
                            publicMixingStationList1.putOpt("thinTwo",bhzCementDetail.getRealityNumber());//    细骨料2
                            publicMixingStationList1.putOpt("thinTwoKg",bhzCementDetail.getTheoryNumber());//细骨料2配比
                            datelist[1]=2;
                        }
                    }
                    if(bhzCementDetail.getMaterialeType()==3){//粗骨料
                        if(datelist[2]==0){
                            publicMixingStationList1.putOpt("thickOne",bhzCementDetail.getRealityNumber());//粗骨料1
                            publicMixingStationList1.putOpt("thickOneKg",bhzCementDetail.getTheoryNumber());//粗骨料1配比
                            datelist[2]=1;
                        }else if(datelist[2]==1){
                            publicMixingStationList1.putOpt("thickTwo",bhzCementDetail.getRealityNumber());//    粗骨料2
                            publicMixingStationList1.putOpt("thickTwoKg",bhzCementDetail.getTheoryNumber());//粗骨料1配比
                            datelist[2]=2;
                        }else if(datelist[2]==2){
                            publicMixingStationList1.putOpt("thickThree",bhzCementDetail.getRealityNumber());//    粗骨料3
                    ...
                    if(bhzCementDetail.getMaterialeType()==5){//水
                        if(datelist[4]==0){
                            publicMixingStationList1.putOpt("waterOne",bhzCementDetail.getRealityNumber());//水1
                            publicMixingStationList1.putOpt("waterOneKg",bhzCementDetail.getTheoryNumber());//水1配比
                            datelist[4]=1;
                        }else if(datelist[4]==1){
                            publicMixingStationList1.putOpt("waterTwo",bhzCementDetail.getRealityNumber());//    水2
                    ...
                    if(bhzCementDetail.getMaterialeType()==6){//水泥
                        if(datelist[5]==0){
                            publicMixingStationList1.putOpt("cementOne",bhzCementDetail.getRealityNumber());//水泥1
                            publicMixingStationList1.putOpt("cementOneKg",bhzCementDetail.getTheoryNumber());//水泥1配比
                            datelist[5]=1;
                        }else if(datelist[5]==1){
                            publicMixingStationList1.putOpt("cementTwo",bhzCementDetail.getRealityNumber());//水泥2
                    ...
                    if(bhzCementDetail.getMaterialeType()==9){//外加剂
                        if(datelist[8]==0){
                            publicMixingStationList1.putOpt("otherAddOne",bhzCementDetail.getRealityNumber());//外加剂1
                            publicMixingStationList1.putOpt("otherAddOneKg",bhzCementDetail.getTheoryNumber());//外加剂1配比
                            datelist[8]=1;
                        }else if(datelist[8]==1){
                            publicMixingStationList1.putOpt("otherAddTwo",bhzCementDetail.getRealityNumber());//外加剂2
                            publicMixingStationList1.putOpt("otherAddTwoKg",bhzCementDetail.getTheoryNumber());//外加剂2配比
                            datelist[8]=2;
                        }else if(datelist[8]==2){
                            publicMixingStationList1.putOpt("otherAddThree",bhzCementDetail.getRealityNumber());//外加剂3
                            publicMixingStationList1.putOpt("otherAddThreeKg",bhzCementDetail.getTheoryNumber());//外加剂3配比
                            datelist[8]=3;
                        }else if(datelist[8]==3){
                            publicMixingStationList1.putOpt("otherAddFour",bhzCementDetail.getRealityNumber());//外加剂4
        ...
        if(publicMixingStationList.size()>0){
            String s1 = shandongUtil.TuisongList(ShandongUtil.TuiSong, token, ShandongUtil.publicMixingList, publicMixingStationList.toString());
            if(s1.equals("200")){
                sysConfigService.updateSysConfig(JobUtil.HNTBHZ_TUISONG_SHANDONGLUKE, id);//根据传过来的唯一值来修改当前判断到的数据id
                //此处修改sys_config 中记录推送到那个id
                log.info(String.format("推送砼拌合站数据成功" + id+DateUtils.now()));
            }else{
                log.info(String.format("推送砼拌合站数据失败" + id+DateUtils.now()));
            }
        }
