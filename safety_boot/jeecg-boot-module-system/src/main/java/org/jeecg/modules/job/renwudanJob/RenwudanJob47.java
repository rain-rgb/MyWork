...
public class RenwudanJob47 implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        for (BhzCementBase selecthntbhzone : selecthntbhzones) {
            ...
            Shigongphb shigongphb1 = shigongphbService.queryoneCode(formulaNo);
            if (shigongphb1 != null && StringUtils.isNotBlank(formulaNo)) {
                    String renwudan = shigongphb1.getRenwudan();
                    if (renwudan != null) {
                        queryselectone = bhzrenwudanService.queryselectlist(renwudan, 4, shigongphb1.getSysOrgCode());
                        if (queryselectone.size() == 0) {
                            Bhzrenwudan rwd = bhzrenwudanService.selectstation(renwudan);
                            if (rwd == null) {
                                log.info(String.format("暂无相匹配的任务单数据" + DateUtils.now()));
                                bhzCementBaseService.updatehntbhzrenwustatus(batchNo, 30);
                                continue;
                            }else{
                                queryselectone.add(rwd);
                            }
                        }
                    } else {
                        ...
                        for (Bhzrenwudan bhzrenwudan : queryselectone) {
                            ...
                            if (queryone == null) {
                                ...
                                if(mete != null){
                                    ...
                                    renwudanSchedule.setBfb(100.00);
                                }
                                ...
                                if (jindu >= 100.0) {
                                    ...
                                    if(one.getMete()<=one.getMetes()){
                                        bhzrenwudans.setIsfinish(1);
                                        bhzrenwudans.setStatus(5);
                                        bhzrenwudans.setMetes(String.valueOf(one.getMetes()));
                                    }else{
                                        bhzrenwudans.setStatus(4);
                                        bhzrenwudans.setMetes(String.valueOf(one.getMetes()));
                                    }
                                ...
                                int updateone = renwudanScheduleService.updateone(Double.parseDouble(format2), jindu, id1, productDatetime, dishCount);
                                if (updateone == 1) {
                                    log.info(String.format("添加任务单进度统计成功" + DateUtils.now()));
                                } else {
                                    log.info(String.format("添加任务单进度统计失败" + DateUtils.now()));
