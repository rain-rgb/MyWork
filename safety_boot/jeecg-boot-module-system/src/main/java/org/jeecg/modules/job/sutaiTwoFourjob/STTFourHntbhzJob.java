package org.jeecg.modules.job.sutaiTwoFourjob;
...
import org.jeecg.modules.job.sutaiTwoFivejob.SuTaiTFUtil;
...
@Slf4j
public class STTFourHntbhzJob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.STTWO_FOURHNTBHZ);//苏台二期四标混凝土拌合站
        //如果他等于空
        if (null == selectsysconfigone) {
            log.info(String.format("未获取到苏台四标拌合站定时任务的配置信息" + DateUtils.now()));
            return;
        }
        //当前数据检测到的目标id
        ...
        if (null == jsonObject || jsonObject.isEmpty()) {
            log.info(String.format("没有配置要传输苏台四标拌合站的设备" + DateUtils.now()));
            return;
        }
        String shebeilist = jsonObject.getStr("shebeilist");
//        String[] split = shebeilist.split(",");
//        List<String> strsToList1 = Arrays.asList(split);
        List<BhzCementBase> bhzCementBasess = iBhzCementBaseService.selectHntbhzListBySTFour(curid, 1, shebeilist);
        if (null == bhzCementBasess || bhzCementBasess.size() == 0) {
            log.info(String.format("暂无苏台四标砼拌合站的数据" + DateUtils.now()));
            return;
        }
        int id = 0;
...
//            saveDTOList.set("equipmentName", "苏台二期五标测试拌合站");//设备名称
            ...
            saveDTOList.set("sectionType", 4);//1 一标 2 二标 3 三标 4 四标 5 五标
        ...
        if (jsonArray.size() > 0) {
            Integer integer = suTaiTFUtil.PostSTTHIBHZList(jsonArray);
            if (integer == 200) {
                log.info(String.format("苏台拌合站二期四标数据推送成功!" + id));
                sysConfigService.updateSysConfig(JobUtil.STTWO_FOURHNTBHZ, id);//根据传过来的唯一值来修改当前判断到的数据id
            } else {
                log.info(String.format("苏台拌合站二期四标数据推送失败!" + id));
            }
        } else {
            log.info(String.format("苏台拌合站当前数据属于二期四标" + id));
            sysConfigService.updateSysConfig(JobUtil.STTWO_FOURHNTBHZ, id);//根据传过来的唯一值来修改当前判断到的数据id
        }
    }
}
