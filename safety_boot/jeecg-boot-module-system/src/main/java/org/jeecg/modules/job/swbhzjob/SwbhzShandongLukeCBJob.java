 ...
 * \* Date: 2021/6/4
 * \* Time: 18:05
 * \* Description:山东陆科推送水稳拌合站超标数据(实时数据包含所有的合格不合格数据 注：推送的数据已经判断过超不超标)
 * \
 */
@Slf4j
@PersistJobDataAfterExecution
@DisallowConcurrentExecution
public class SwbhzShandongLukeCBJob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.SWBHZ_TUISONG_SHANDONGLUKECB);//水稳拌合站超标数据推送山东陆科
        //如果他等于空
        ...
        List<BhzSwBases> selecthntbhzones = bhzSwBasesService.selectswbhzcblist(curid, 1, sbjnolist,0);//超标数据
        ...
        JSONArray publicWaterMoreList=new JSONArray();//传的参数的格式为[{},{}]
        for (BhzSwBases selecthntbhzone : selecthntbhzones) {
            ...
            publicMixingStationList1.putOpt("mixerId",selecthntbhzone.getId());//水稳生产实时数据id
...
//            }
            publicMixingStationList1.putOpt("standardType",selecthntbhzone.getChaobiaodengji());//    超标类型
            ...
            if(selectswbhzcailiao.size()>0){
                ...
                for (BhzSwCailiao bhzCementDetail : selectswbhzcailiao) {
                    ...
                    if((bhzCementDetail.getCailiaoleixing()==1||bhzCementDetail.getCailiaoleixing()==2||
                            bhzCementDetail.getCailiaoleixing()==3||bhzCementDetail.getCailiaoleixing()==4)&&bhzCementDetail.getChaobiaodengji()>0){//实际骨料1-实际骨料5 理论骨料1-理论骨料5
                        if(datelist[2]==0){
                            publicMixingStationList1.putOpt("aggregateOne",bhzCementDetail.getChaobiaodengji());//实际骨料1
                            datelist[2]=1;
                        }else if(datelist[2]==1){
                            publicMixingStationList1.putOpt("aggregateTwo",bhzCementDetail.getChaobiaodengji());//    实际骨料2
                            datelist[2]=2;
                        }else if(datelist[2]==2){
                            publicMixingStationList1.putOpt("aggregateThree",bhzCementDetail.getChaobiaodengji());//    实际骨料3
                            datelist[2]=3;
                        }else if(datelist[2]==3){
                            publicMixingStationList1.putOpt("aggregateFour",bhzCementDetail.getChaobiaodengji());//    实际骨料4
                            datelist[2]=4;
                        }else if(datelist[2]==4){
                            publicMixingStationList1.putOpt("aggregateFive",bhzCementDetail.getChaobiaodengji());//    实际骨料5
                            datelist[2]=5;
                        }
                    }
                    if(bhzCementDetail.getCailiaoleixing()==5&&bhzCementDetail.getChaobiaodengji()>0){//实际水/理论水
                        if(datelist[4]==0){
                            publicMixingStationList1.putOpt("water",bhzCementDetail.getChaobiaodengji());//实际水
                            datelist[4]=1;
                        }
                    }
                    if(bhzCementDetail.getCailiaoleixing()==8&&bhzCementDetail.getChaobiaodengji()>0){//实际粉料1/理论粉料1/实际粉料2/理论粉料2
                        if(datelist[7]==0){
                            publicMixingStationList1.putOpt("powderOne",bhzCementDetail.getChaobiaodengji());//实际粉料1
                            datelist[7]=1;
                        }else if(datelist[7]==1){
                            publicMixingStationList1.putOpt("powderTwo",bhzCementDetail.getChaobiaodengji());//实际粉料2
                            datelist[7]=2;
                        }
                    }
                }
            }
            publicWaterMoreList.add(publicMixingStationList1);
        }
        if(publicWaterMoreList.size()>0){
            String s1 = shandongUtil.TuisongList(ShandongUtil.TuiSong, token, ShandongUtil.publicWaterMoreList, publicWaterMoreList.toString());
            if(s1.equals("200")){
                sysConfigService.updateSysConfig(JobUtil.SWBHZ_TUISONG_SHANDONGLUKECB, id);//根据传过来的唯一值来修改当前判断到的数据id
                //此处修改sys_config 中记录推送到那个id
                log.info(String.format("推送水稳拌合站超标数据成功" + id+DateUtils.now()));
            }else{
                log.info(String.format("推送水稳拌合站超标数据失败" + id+DateUtils.now()));
            }
        }
