...
/**
 * 统计料仓累计进场jinchanggbMan表
 */
@Slf4j
@PersistJobDataAfterExecution
@DisallowConcurrentExecution
public class LiCangTJinChangManJob implements Job {
    @Autowired
    private ISysConfigService sysConfigService;
    @Autowired
    private WzliaocangMapper wzliaocangMapper;
    @Autowired
    private WzycljinchanggbManMapper wzycljinchanggbMapper;

    @Override
    @Transactional
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.TJ_LIAOCANG_JCMAN);//查询料仓进场总数之前统计id
        //如果他等于空
        ...
        QueryWrapper<WzycljinchanggbMan> wrapper = new QueryWrapper<>();
        wrapper.select("max(id) id, SUM(CAST(jingzhong AS DECIMAL(10,5))) jingzhong, lcbianhao").gt("id", curid).groupBy("LCbianhao").orderByDesc("id");
        List<WzycljinchanggbMan> wzycljinchanggbs = wzycljinchanggbMapper.selectList(wrapper);
        if (oConvertUtils.isEmpty(wzycljinchanggbs) || wzycljinchanggbs.size() == 0) {
            log.info("没有要新的数据统计");
            return;
        }
        for (WzycljinchanggbMan wzycljinchanggb : wzycljinchanggbs) {
            if (oConvertUtils.isEmpty(wzycljinchanggb.getLcbianhao()) || oConvertUtils.isEmpty(wzycljinchanggb.getJingzhong())) {
                continue;
            }
            Double jinzhong = Double.valueOf(wzycljinchanggb.getJingzhong());
