...
public class BhzStatisticsJob implements Job {
    @Autowired
...
//        }
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.BHZ_STATISTICS);
        //如果他等于空
...
//        }
        ...
        List<BhzCementBase> selecthntbhzones = bhzCementBaseService.selecthntbhzonesstatistics(curid, 0);
        //如果他等于空
...
//        sysConfigService.updateSysConfigs(JobUtil.BHZ_STATISTICS, curid,1);
        for (BhzCementBase selecthntbhzone : selecthntbhzones) {
            if (selecthntbhzone.getAlertstate() == 0) {
                ...
                log.info("该拌合站数据没有超标检测id:"+selectsysconfigone2.getCurid()+"时间" + DateUtils.now());
        ...
        sysConfigService.updateSysConfigs(JobUtil.BHZ_STATISTICS, id, 0);//根据传过来的唯一值来修改当前判断到的数据id
    }
}
