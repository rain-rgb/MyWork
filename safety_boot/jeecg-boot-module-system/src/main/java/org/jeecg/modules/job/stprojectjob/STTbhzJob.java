 ...
 */
@Slf4j
public class STTbhzJob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.ST_TBHZ);//苏台拌合站
        //如果他等于空
...
//        List<String> strsToList1= Arrays.asList(split);
        List<BhzCementBase> bhzCementBasess = iBhzCementBaseService.selectHntbhzList(curid, 1, shebeilist);
...
//            ShebeiInfo selectshebeione = shebeiInfoService.selectshebeione(bhzCementBases.getShebeiNo());
            ...
            if (bhzCementBases.getShebeiNo().equals("stgsbh02_01")) {
                saveDTOList.set("equipmentName", "TJ01标1号拌和机");//设备名称
            } else if (bhzCementBases.getShebeiNo().equals("stgsbh02_02")) {
                saveDTOList.set("equipmentName", "TJ01标2号拌和机");//设备名称
            } else if (bhzCementBases.getShebeiNo().equals("stgsbh01_03")) {
                saveDTOList.set("equipmentName", "TJ01标3号拌和机");//设备名称
            }
            ...
            for (BhzCementDetail bhzCementDetail : bhzCementDetails) {
                ...
                //saveDTOList1.set("concreteStandId",bhzCementDetail.getBatchNo());//主表Id
        ...
        if (jsonArray.size() > 0) {
            Integer integer = stUtil.PostSTTBHZList(jsonArray);
            if (integer == 200) {
                log.info(String.format("苏台拌合站数据推送成功!" + id));
                sysConfigService.updateSysConfig(JobUtil.ST_TBHZ, id);//根据传过来的唯一值来修改当前判断到的数据id
            } else {
                log.info(String.format("苏台拌合站数据推送失败!" + id));
            }
        } else {
