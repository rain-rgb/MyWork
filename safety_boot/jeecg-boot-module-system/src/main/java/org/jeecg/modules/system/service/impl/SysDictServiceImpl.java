...
public class SysDictServiceImpl extends ServiceImpl<SysDictMapper, SysDict> implements ISysDictService {
	...
	@Override
	public Map<String, List<DictModel>> queryMayDictItems(String dictCode) {
		Map<String, List<DictModel>> res = new HashMap<String, List<DictModel>>();
		QueryWrapper<SysDict> queryWrapper1 = new QueryWrapper<>();
		queryWrapper1.eq("dict_code",dictCode);
		List<SysDict> ls = sysDictMapper.selectList(queryWrapper1);
		LambdaQueryWrapper<SysDictItem> queryWrapper = new LambdaQueryWrapper<SysDictItem>();
		queryWrapper.eq(SysDictItem::getStatus, 1);
		queryWrapper.orderByAsc(SysDictItem::getSortOrder);
		List<SysDictItem> sysDictItemList = sysDictItemMapper.selectList(queryWrapper);
	...
	@Override
	public SysDictItem selectdictbyid(String prontzhi, int j) {
		QueryWrapper<SysDict> queryWrapper = new QueryWrapper<>();
		queryWrapper.eq("dict_code",prontzhi);
		SysDict sysDict = sysDictMapper.selectOne(queryWrapper);

		QueryWrapper<SysDictItem> queryWrapper1 = new QueryWrapper<>();
		queryWrapper1.eq("dict_id",sysDict.getId());
		queryWrapper1.eq("item_value",j);
		SysDictItem sysDictItem = sysDictItemMapper.selectOne(queryWrapper1);
		return sysDictItem;
	}
	...
	@Override
	public SysDict selectdict(String dictCode) {
		try{
			QueryWrapper<SysDict> queryWrapper = new QueryWrapper<>();
