...
@Slf4j
public class STTFHntbhzJob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.STTWO_FIVEHNTBHZ);//苏台二期五标混凝土拌合站
        //如果他等于空
...
//            saveDTOList.set("equipmentName", "苏台二期五标测试拌合站");//设备名称
        ...
        if (jsonArray.size() > 0) {
            ...
            if (integer == 200) {
                log.info(String.format("苏台拌合站二期五标数据推送成功!" + id));
                sysConfigService.updateSysConfig(JobUtil.STTWO_FIVEHNTBHZ, id);//根据传过来的唯一值来修改当前判断到的数据id
            } else {
                log.info(String.format("苏台拌合站二期五标数据推送失败!" + id));
            }
        } else {
            log.info(String.format("苏台拌合站当前数据属于二期五标" + id));
            sysConfigService.updateSysConfig(JobUtil.STTWO_FIVEHNTBHZ, id);//根据传过来的唯一值来修改当前判断到的数据id
        }
    }
}
