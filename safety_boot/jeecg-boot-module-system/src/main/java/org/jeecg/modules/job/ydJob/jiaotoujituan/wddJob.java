...
@Service
public class wddJob implements Job {
    @Autowired
...
//根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.JTJT_WDD);//稳定度数据推送交投集团
        //如果他等于空
        ...
        List<WendingD> wendingDS = wendingDService.getListjt(shebeilist, curid);
        if (null == wendingDS || wendingDS.isEmpty()) {
            log.info(String.format("没有要传输稳定度数据的数据" + DateUtils.now()));
            return;
        }
        int id = 0;
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); // 时间格式化器
        DecimalFormat df = new DecimalFormat("#.00");
        for (WendingD wendingD : wendingDS) {
            id = wendingD.getId();
            JSONObject sendJSONObject = JSONUtil.createObj();
            String fsbbh = wendingD.getFsbbh();
            String deviceNum = "RC_TJ01_GDLQMXE_01";
            if (fsbbh.equals("rcgs04wdd01")) {
                deviceNum = "RC_TJ04_GDLQMXE_01";
            } else if (fsbbh.equals("ytwtzbdzswdd001")){
                deviceNum = "YTW_TJ03_GDLQMXE_02";
            }
            sendJSONObject.put("deviceNum", deviceNum);
            Integer sampleType = 0;
            Integer lqtype = wendingD.getLqtype();
            ...
            sendJSONObject.put("sampleName", wendingD.getSamplename());
            sendJSONObject.put("sampleNum", wendingD.getSampleno());
            sendJSONObject.put("sampleDes", wendingD.getSamplems());
            String submittime = wendingD.getSubmittime();
            sendJSONObject.put("trialDate", submittime);
            sendJSONObject.put("proSection", wendingD.getGcbuwei());//工程部位
            sendJSONObject.put("testPerson", "");//试验人员
            String isqualified = wendingD.getIsqualified();
            if (isqualified.equals("1")||isqualified.equals("不合格")){
                sendJSONObject.put("isQualified", 1);
            } else {
                sendJSONObject.put("isQualified", 0);
            }
            sendJSONObject.put("precativeNum","");
            sendJSONObject.put("stabilityValue",getData(wendingD.getWddavg(),0));
            sendJSONObject.put("flowValue",getData(wendingD.getLiuzhiavg(),1));
            sendJSONObject.put("stabilityStandard",getData(wendingD.getBiaozhunzhi1(),0) + "-" + getData(wendingD.getBiaozhunzhi2(),0));

            sendJSONObject.put("flowStandard",">=" + getData(wendingD.getBiaozhunzhi3(),0));
            ...
            String body = HttpRequest.post("http://47.110.39.210:6023/api/asphaltExperiment/saveApiTrialMxe")
