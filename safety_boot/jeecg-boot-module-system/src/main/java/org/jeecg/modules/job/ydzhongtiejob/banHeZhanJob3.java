...
public class banHeZhanJob3 implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.YB_GTBHZ3);//开投
        //如果他等于空
        ...
        for (BhzCementBase bhzCementBase : bhzCementBases) {
            ...
            if (strengthRank == null || "bp4bhnt-02".equals(shebeiNo)){
                strengthRank = bhzCementBase.getFormulaNo();
            }
            try {
                String s = strengthRank.substring(0, 3).toUpperCase();
                sendDate.set("strength", s);
            } catch (Exception e) {
            ...
            bhzCementBase1.setId(id);
            if ("200".equals(codes)) {
                log.info(String.format("开投推送成功!" + id + "Json数据" + sendDates));
                bhzCementBase1.setIssend(1);
            } else {
                bhzCementBase1.setIssend(2);
            }
            bhzCementBaseService.updateById(bhzCementBase1);
            sysConfigService.updateSysConfig(JobUtil.YB_GTBHZ3, id);//根据传过来的唯一值来修改当前判断到的数据id
        }
    }
}
