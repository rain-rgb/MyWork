...
public class deviceBridgeHistorydataJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.DEVICE_BRIDGE_HIS);//集团标准版架桥机历史数据推送
        //如果他等于空
        if (null == selectsysconfigone) {
            log.info(String.format("未获取到集团标准版架桥机历史数据推送定时任务的配置信息" + DateUtils.now()));
            return;
        }
        //当前数据检测到的目标id
        ...
        if (null == jsonObject || jsonObject.isEmpty()) {
            log.info(String.format("没有配置要传输集团标准版架桥机历史数据推送的组织机构" + DateUtils.now()));
            return;
        }
        ...
        if (shebeiList.size() == 0) {
            log.info(String.format("没有配置要传输集团标准版架桥机历史数据推送的设备" + DateUtils.now()));
            return;
        }
        ...
        List<DeviceBridgeHistorydata> realdataList = deviceBridgeHistorydataService.list(queryWrapper);
        if (null == realdataList || realdataList.size() == 0) {
            log.info(String.format("暂无集团标准版架桥机历史数据推送的数据" + DateUtils.now()));
            return;
        }
        int id = 0;
        for (DeviceBridgeHistorydata data : realdataList) {
            id = data.getId();
            ...
            if (code1.equals("00000")) {
                ...
                String post = HttpRequest.post("https://yggc.cncico.com:1080/api/iot/dxsb/bridgeHistorydatas")
                        .header("Content-Type", "application/json")
                ...
                if(code2.equals("00000")){
                    log.info(String.format("集团标准版架桥机历史数据推送成功!" + sendObject));
                    sysConfigService.updateSysConfig(JobUtil.DEVICE_BRIDGE_HIS, id);
                } else {
                    log.info(String.format("集团标准版架桥机历史数据推送失败!" + sendObject));
                }
            }
        }
    }
