...
public class SGBanhezhanJob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.SG_BHZ);//疏港超标数据推送浙路品质
        //如果他等于空
        ...
        List<BhzCementBase> bhzCementBases = bhzCementBaseService.selectListsbhgsg(shebeilist, curid);
        ...
        for (BhzCementBase bhzCementBase : bhzCementBases) {
            ...
            if (null != one1) {
                ...
                sendDate.set("attachment", one1.getFilePath());//附件
            }
            ...
            if (result == 0) {
                ...
                log.info(String.format("浙路品质疏港砼拌合站推送成功！%s", id));
            } else {
                bhzCementBase.setIssend(2);
                log.info(String.format("浙路品质疏港砼拌合站推送失败！%s", id));
            }
