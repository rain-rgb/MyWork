...
public class SysDepartServiceImpl extends ServiceImpl<SysDepartMapper, SysDepart> implements ISysDepartService {
	...
	@Override
	public List<SysDepartTreeModel> queryTreeListByOrgCode() {
		LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
		LambdaQueryWrapper<SysDepart> query = new LambdaQueryWrapper<SysDepart>();
		query.eq(SysDepart::getDelFlag, CommonConstant.DEL_FLAG_0.toString());
		query.likeRight(SysDepart::getOrgCode, user.getOrgCode());
		query.orderByAsc(SysDepart::getDepartOrder);
		List<SysDepart> list = this.list(query);
		// 调用wrapTreeDataToTreeList方法生成树状数据
		List<SysDepartTreeModel> listResult = FindsDepartsChildrenUtil.wrapTreeDataToTreeList(list);
	...
	@Override
	public SysDepart queryone(String sysorgcode) {
		LambdaQueryWrapper<SysDepart> lambdaQueryWrapper=new LambdaQueryWrapper<>();
		lambdaQueryWrapper.eq(SysDepart::getOrgCode,sysorgcode);
		return this.getOne(lambdaQueryWrapper);
	}

	@Override
	public SysDepart querynameabbrList(String depart_name, String depart_name_abbr) {
		LambdaQueryWrapper<SysDepart> lambdaQueryWrapper=new LambdaQueryWrapper<>();
		lambdaQueryWrapper.eq(SysDepart::getDepartNameAbbr,depart_name_abbr);
		lambdaQueryWrapper.like(SysDepart::getDepartName,depart_name);
		return this.getOne(lambdaQueryWrapper);
	}

	@Override
	public List<SysDepart> querydepart_name_abbrList(String depart_name_abbr) {
		LambdaQueryWrapper<SysDepart> lambdaQueryWrapper=new LambdaQueryWrapper<>();
		lambdaQueryWrapper.eq(SysDepart::getDepartNameAbbr,depart_name_abbr);
	 ...
	 */
	@Override
	public List<SysDepart> selectqueryorgcode(String departIds) {
		LambdaQueryWrapper<SysDepart> queryWrapper=new LambdaQueryWrapper<>();
		queryWrapper.in(SysDepart::getId,Arrays.asList(departIds.split(",")));
		queryWrapper.orderByAsc(SysDepart::getOrgCode);
		List<SysDepart> list=this.list(queryWrapper);
		return list;
	}

	@Override
	public List<SysDepart> getsysDepartlist(String id) {
		try{
			LambdaQueryWrapper<SysDepart> queryWrapper=new LambdaQueryWrapper<>();
			queryWrapper.eq(SysDepart::getParentId,id);
	...
	@Override
	public List<SysDepart> getsysDepartlists(String id, String orgCategory) {
		try{
			LambdaQueryWrapper<SysDepart> queryWrapper=new LambdaQueryWrapper<>();
			queryWrapper.likeRight(SysDepart::getOrgCode,id);
			queryWrapper.eq(SysDepart::getOrgCategory,orgCategory);
			queryWrapper.eq(SysDepart::getDelFlag,"0");
			return this.list(queryWrapper);
		} catch (Exception e) {
			e.printStackTrace();
	...
	@Override
	public SysDepart selectbyorgcode(String sysOrgCode, int i) {
		try{
			LambdaQueryWrapper<SysDepart> queryWrapper=new LambdaQueryWrapper<>();
			queryWrapper.likeRight(SysDepart::getOrgCode,sysOrgCode);
			queryWrapper.eq(SysDepart::getOrgCategory,i);
    ...
    @Override
    public SysDepart selectByCode(String sysOrgCode) {
		return sysDepartMapper.selectByCode(sysOrgCode);
    }

	@Override
	public List<SysDepart> selectBySBNo() {
		return sysDepartMapper.selectBySBNo();
	}

	@Override
	public List<String> fingAllSheBeiNo(String orgCode) {
		return sysDepartMapper.fingAllSheBeiNo(orgCode);
	}

	@Override
	public SysDepart queryoneByproject(String projectid, String sectionid) {
		LambdaQueryWrapper<SysDepart> queryWrapper=new LambdaQueryWrapper<>();
		if(StringUtils.isNotBlank(sectionid)){
			queryWrapper.likeRight(SysDepart::getSectionid,sectionid);
			queryWrapper.eq(SysDepart::getOrgCategory,7);
		}else{
			queryWrapper.likeRight(SysDepart::getProjectid,projectid);
			queryWrapper.eq(SysDepart::getOrgCategory,4);
		}
