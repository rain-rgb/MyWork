...
public class BanhezhanJob implements Job {
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.RC_BHZ);//瑞苍拌合站
        //如果他等于空
        ...
        List<BhzCementBase> bhzCementBases = bhzCementBaseService.selectListsbhg(shebeilist, curid);
        ...
        for (BhzCementBase bhzCementBase : bhzCementBases){
            ...
            if (null == one){
                log.info("设备未注册！"+ bhzCementBase.getShebeiNo());
                continue;
            }
            ...
            Integer overproofStatus = bhzCementBase.getOverproofStatus();
            Integer alarmStatus = 0;
            if (overproofStatus>=10){
                alarmStatus = 1;
            }else {
                alarmStatus = 0;
            }
            ...
            List<BhzCementOverHandler> list = bhzCementOverHandlerService.list(queryWrapper);

            for (BhzCementOverHandler bhzCementOverHandler : list) {
                sendDate.set("opinion", bhzCementOverHandler.getProblemReasons());
                sendDate.set("closeTime", bhzCementOverHandler.getHandleTime());
                sendDate.set("closer", bhzCementOverHandler.getHandlePerson());
                String filePath = bhzCementOverHandler.getFilePath();
                sendDate.set("attachment", (filePath != null && !"".equals(filePath)) ? filePath : "");
            ...
            if (result == 0) {
                ...
                log.info(String.format("浙路品质瑞苍砼拌合站推送成功！%s", id));
            } else {
                bhzCementBase.setIssend(2);
                log.info(String.format("浙路品质瑞苍砼拌合站推送失败！%s", id));
            }
