...
public class RCUnifiedProcessYajiangJob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.YJ_UNIDIED_PROCESS);//压浆统一待办
        //如果他等于空
        if (null == selectsysconfigone) {
            log.info(String.format("未获取到压浆统一待办定时任务的配置信息" + DateUtils.now()));
            return;
        }
        //当前数据检测到的目标id
        ...
        if (null == jsonObject || jsonObject.isEmpty()) {
            log.info(String.format("没有配置要传输压浆统一待办推送的设备" + DateUtils.now()));
            return;
        }
        ...
        if (shebeiList == null || shebeiList.size() == 0) {
            log.info(String.format("暂无压浆统一待办推送的设备：没有配置代办人的信息" + DateUtils.now()));
            return;
        }
        Integer alertstate = 1;
        Integer overLevel = 0;
        List<TrYajiangM> yajiangMList = trYajiangMService.selectUnifiedProcess(curid, alertstate, shebeiList, overLevel);
        if (null == yajiangMList || yajiangMList.size() == 0) {
            log.info(String.format("暂无压浆统一待办的数据" + DateUtils.now()));
            return;
        }
        int id = 0;
...
//            unifiedProcessAddReqVO.setStartUserId(0000);
            ...
            unifiedProcessAddReqVO.setProcessId(yajiangM.getSyjid());
            ShebeiInfo selectshebeione = shebeiInfoService.selectshebeione(yajiangM.getYjsbbh());
...
//            unifiedProcessAddReqVO.setPostName("无");
            ...
            BhzCallCfg bhzCallCfg = bhzCallCfgService.selectbhzcallone(yajiangM.getYjsbbh());
            ...
            sysConfigService.updateSysConfig(JobUtil.YJ_UNIDIED_PROCESS, id);
        }
    ...
    public void addUnifiedTask(unifiedTaskAddReqVO unifiedTaskAddReqVO, String token) throws JsonProcessingException {
        ...
        if (code2.equals("00000")) {
            log.info(String.format("压浆统一任务待办添加成功!" + unifiedTaskAddReqVO));
        } else {
            log.info(String.format("压浆统一任务待办添加失败!" + unifiedTaskAddReqVO));
        }
    }
