...
@RestController
@RequestMapping("/RCUnifiedProcessSWJob")
public class RCUnifiedProcessSWJob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.SW_UNIDIED_PROCESS);//水稳拌合站统一待办
        //如果他等于空
        if (null == selectsysconfigone) {
            log.info(String.format("未获取到水稳拌合站统一待办定时任务的配置信息" + DateUtils.now()));
            return;
        }
        //当前数据检测到的目标id
        ...
        if (null == jsonObject || jsonObject.isEmpty()) {
            log.info(String.format("没有配置要传输水稳拌合站统一待办推送的设备" + DateUtils.now()));
            return;
        }
        ...
        if (shebeiList == null || shebeiList.size() == 0) {
            log.info(String.format("暂无水稳拌合站统一待办推送的设备：没有配置代办人的信息" + DateUtils.now()));
            return;
        }
        Integer alertstate = 1;
        Integer overLevel = 0;
        List<BhzSwBases> bhzSwBasesList = bhzSwBasesService.selectBHZUnifiedProcess(curid, alertstate, shebeiList, overLevel);
        if (null == bhzSwBasesList || bhzSwBasesList.size() == 0) {
            log.info(String.format("暂无水稳拌合站统一待办的数据" + DateUtils.now()));
            return;
        }
        int id = 0;
...
//            unifiedProcessAddReqVO.setStartUserId(0000);
            ...
            unifiedProcessAddReqVO.setProcessId(bhzSwBases.getGuid());
...
//            unifiedProcessAddReqVO.setPostName("无");
            ...
            BhzCallCfg bhzCallCfg =  bhzCallCfgService.selectbhzcallone(bhzSwBases.getShebeibianhao());

            unifiedProcessAddReqVO.setPcUrl("http://47.96.161.157/jeecg-boot/sys/systems/ssoredirect/loginSs/bhzxx?username=iot&content="+bhzSwBases.getGuid());
            ...
            if (code1.equals("00000")) {
                ...
                if (code2.equals("00000")) {
                    ...
                    chuzhi.setTaskId(bhzSwBases.getGuid() + "-10");
                    chuzhi.setProjectId(queryone.getProjectid());
                    chuzhi.setSectionId(queryone.getSectionid());
                    chuzhi.setProcessId(bhzSwBases.getGuid());
                    chuzhi.setProcessName(selectshebeione.getSbname() + "超标预警");
                    chuzhi.setStartTime(simpleDateFormat.format(new Date()));
                    chuzhi.setPcRedirectUrl("http://47.96.161.157/jeecg-boot/sys/systems/ssoredirect/loginSs/bhzcz?username="+userByName.getUsername()+"&content="+bhzSwBases.getGuid());
                    ...
                    jianliSP.setTaskId(bhzSwBases.getGuid() + "-40");
                    jianliSP.setProjectId(queryone.getProjectid());
                    jianliSP.setSectionId(queryone.getSectionid());
                    jianliSP.setProcessId(bhzSwBases.getGuid());
                    jianliSP.setProcessName(selectshebeione.getSbname() + "超标预警");
                    jianliSP.setStartTime(simpleDateFormat.format(new Date()));
                    jianliSP.setPcRedirectUrl("http://47.96.161.157/jeecg-boot/sys/systems/ssoredirect/loginSs/bhzsh?username="+userByName.getUsername()+"&content="+bhzSwBases.getGuid());
                    ...
                    zhbSP.setTaskId(bhzSwBases.getGuid() + "-50");
                    zhbSP.setProjectId(queryone.getProjectid());
                    zhbSP.setSectionId(queryone.getSectionid());
                    zhbSP.setProcessId(bhzSwBases.getGuid());
                    zhbSP.setProcessName(selectshebeione.getSbname() + "超标预警");
                    zhbSP.setStartTime(simpleDateFormat.format(new Date()));
                    zhbSP.setPcRedirectUrl("http://47.96.161.157/jeecg-boot/sys/systems/ssoredirect/loginSs/bhzsp?username="+userByName.getUsername()+"&content="+bhzSwBases.getGuid());
                    ...
                    zhbSP.setTaskStatus(0);
                    switch (bhzSwBases.getChaobiaodengji()) {
                        case 1:
            ...
            sysConfigService.updateSysConfig(JobUtil.SW_UNIDIED_PROCESS, id);
        }
    ...
    public void addUnifiedTask(unifiedTaskAddReqVO unifiedTaskAddReqVO, String token) throws JsonProcessingException {
        ...
        if (code2.equals("00000")) {
            log.info(String.format("水稳拌合站统一任务待办添加成功!" + unifiedTaskAddReqVO));
        } else {
            log.info(String.format("水稳拌合站统一任务待办添加失败!" + unifiedTaskAddReqVO));
        }
    }
