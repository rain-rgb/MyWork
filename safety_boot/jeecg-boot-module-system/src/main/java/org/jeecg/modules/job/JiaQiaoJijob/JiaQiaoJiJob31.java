...
public class JiaQiaoJiJob31 implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.NB_JIAQIAOJI31);//通用门式起重机30
        //如果他等于空
        ...
        for (DeviceCraneHistorydata selecthntbhzone : selecthntbhzones) {
            ...
            if (deviceCode.equals("2021111202")) {
                unifiedCode = "360808";
            }
        }
        ...
        if (null != nBaccessToken) {
            ...
            if (integer == 200) {
                log.info(String.format("宁波市特检察院(通用门式起重机+360808)数据推送成功!" + id + "Json数据" + sendData));
            } else {
                log.info(String.format("宁波市特检察院(通用门式起重机+360808)数据推送失败!" + id + "Json数据" + sendData));
            }
        } else {
            log.info(String.format("宁波市特检察院(通用门式起重机+360808)获取token失败!"));
        }
        sysConfigService.updateSysConfig(JobUtil.NB_JIAQIAOJI31, id);//根据传过来的唯一值来修改当前判断到的数据id
    }
}
