...
import com.qiyuesuo.sample.api.utils.StringUtils;
...
import org.jeecg.modules.job.stprojectjob.STtufangysJob;
...
public class tfysworkareaJob implements Job {

    private static String workAreaurl = "https://dp.huace.cn/digitalPlatform/open/api/roadbed/workArea.shtml";
    private static String stationDetailurl = "https://dp.huace.cn/digitalPlatform/open/api/roadbed/stationDetail.shtml";
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        for (HcSection hcSection : sectionList) {
            ...
            HcToken hcToken = null;
            String token = null;
            JSONObject sendDate = null;
            String result = null;
            try {
                QueryWrapper<HcToken> tokenQueryWrapper = new QueryWrapper<>();
                    tokenQueryWrapper.like("projectid", pjid);
                hcToken = tokenService.getOne(tokenQueryWrapper);
                token = hcToken.getToken();

                QueryWrapper<HcTfysworkarea> tfysworkareaQueryWrapper = new QueryWrapper<>();
                tfysworkareaQueryWrapper.select("max(starttime) starttime")
                        .eq("sectionid", sectionid)
                        .eq("workstat", "1");
                HcTfysworkarea one = tfysworkareaService.getOne(tfysworkareaQueryWrapper);
                // 优化后的代码
                String startTimestr = "2023-01-01 00:00:01";
                if (one != null) {
                    startTimestr = sdf.format(one.getStarttime());
                }
...
//            sendDate.put("startTime", "2023-11-29 00:00:01");

                result = HttpRequest.post(workAreaurl)
                        .header("token", token)
                        .body(String.valueOf(sendDate))
                        .execute()
                        .body();
                if(result.contains("token不合法")){
                    QuartzJob quartzJob = quartzJobService.getById("1662535472323596290");
                    quartzJobService.execute(quartzJob);
                    return;
                }
            } catch (HttpException e) {
                pushandreturnService.saveData(1,"代码报错","工作区施工成果明细（华测）", String.valueOf(e));
            }
            ...
            log.info("-----------剩余总条数：" + data1.getInteger("rowCount"));

            for (int i = 1; i <= pageCount; i++) {
                token = hcToken.getToken();
                sendDate.put("pageNo", i);
                String back = HttpRequest.post(workAreaurl)
                        .header("token", token)
                        .body(String.valueOf(sendDate))
                        .execute()
                        .body();
                ...
                for (int j = 0; j < records.size(); j++) {
                    ...
                    HcTfysworkarea tfysworkarea = jsonObject.toJavaObject(jsonObject1, HcTfysworkarea.class);
                    String blockid = tfysworkarea.getBlockid();
                    Date starttime = tfysworkarea.getStarttime();

                    QueryWrapper<HcTfysworkarea> tfysworkareaQueryWrapper1 = new QueryWrapper<>();
                    tfysworkareaQueryWrapper1.eq("sectionid", sectionid);
                    tfysworkareaQueryWrapper1.last(" AND (blockId = '" + blockid + "' or starttime = '" + sdf.format(starttime) + "')");
                    HcTfysworkarea one1 = tfysworkareaService.getOne(tfysworkareaQueryWrapper1);
                    if (one1 != null) {
                        if ("1".equals(one1.getWorkstat())){
                            continue;
                        }
                        tfysworkarea.setId(one1.getId());
                        tfysworkareaService.updateById(tfysworkarea);

                        log.info("----------土方压实工作区施工状态已改变！");
                    } else {
                        tfysworkarea.setProjectid(pjid);
                        tfysworkarea.setSectionid(sectionid);
                        tfysworkareaService.save(tfysworkarea);

                        log.info("----------土方压实工作区施工成果明细添加成功！");
                    }
                    if ("1".equals(tfysworkarea.getWorkstat())) {
                        JSONObject sendDatesta = new JSONObject();
                        sendDatesta.put("projectId", pjid);
                        sendDatesta.put("sectionId", sectionid);
                        sendDatesta.put("blockId", blockid);

                        String back1 = HttpRequest.post(stationDetailurl)
                                .header("token", token)
                                .body(String.valueOf(sendDatesta))
                                .execute()
                                .body();
                        JSONObject jsonObjectsta = JSONObject.parseObject(back1);
                        Integer codesta = jsonObjectsta.getInteger("code");
                        if (0 != codesta) {
                            continue;
                        }
                        JSONArray datasta = jsonObjectsta.getJSONArray("data");
                        for (int k = 0; k < datasta.size(); k++) {
                            JSONObject stajsonObject = datasta.getJSONObject(k);
                            HcTfstationdetail tfstationdetail = jsonObject.toJavaObject(stajsonObject, HcTfstationdetail.class);
                            tfstationdetail.setBlockid(blockid);
                            tfstationdetail.setSectionid(sectionid);
                            tfstationdetailService.save(tfstationdetail);
                            log.info("----------土方压实工作区逐桩详情添加成功！");
                        }
                    }
                }
            }
...
//        obj.put("time", sdf.format(date));//推送时间
        ...
        String format1 = sdf.format(getEndtime);
        if (!format1.equals(format)){//如果不是本日不推送，反之是本日推送
            b = true;
        }else {
