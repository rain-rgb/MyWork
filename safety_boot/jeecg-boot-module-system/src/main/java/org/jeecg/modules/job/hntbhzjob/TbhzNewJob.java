...
public class TbhzNewJob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        List<BhzCementBase> selecthntbhzones = bhzCementBaseService.selecthntbhzonesx(curid, 0);
        ...
        for (BhzCementBase selecthntbhzone : selecthntbhzones) {
            ...
            String shebeiNo1 = selecthntbhzone.getShebeiNo();
            if(projectName != null && shebeiNo1 != null) {
                LambdaQueryWrapper<Bhzshaixuan> queryWrapper = new LambdaQueryWrapper<>();
                queryWrapper.eq(Bhzshaixuan::getShebeino, shebeiNo1);
                List<Bhzshaixuan> list = bhzshaixuanService.list(queryWrapper);
                for (Bhzshaixuan bhzshaixuan : list) {
                    String peojectname = bhzshaixuan.getPeojectname();
                    if (peojectname != null) {
                        if (!projectName.contains(peojectname)) {
                            selecthntbhzone.setShebeiNo(shebeiNo1+"sc");
                            selecthntbhzone.setAlertstate(1);
                            bhzCementBaseService.updateById(selecthntbhzone);
                        }

                    }
                    String description = bhzshaixuan.getDescription();
                    if (description!= null) {
                        if (projectName.contains(description)||poureLocation.contains(description)) {
                            selecthntbhzone.setShebeiNo(shebeiNo1+"sc");
                            selecthntbhzone.setAlertstate(1);
                            bhzCementBaseService.updateById(selecthntbhzone);
                        }
                ...
                if (selecthntbhzone.getAlertstate()==1) {
                    continue;
                }
            }
            ...
            try {
                ...
                String uid = "43d829206e7f4c338819ed38570b91bf";
                if (null != selectbhzcallone) {
                    uid = selectbhzcallone.getUid();
                }
                List<BhzChaobiaoCfg> selectchaobiaolist = bhzChaobiaoCfgService.selectchaobiaolist(shebeiNo,uid);
...
//                }
                ...
                if (selectshebeione != null) {
                    if (selectshebeione.getIspd() != null && selectshebeione.getIspd() == 1) {
                        Field[] fields = selecthntbhzone.getClass().getDeclaredFields();
                        for (Field field : fields) {
                            // 设置字段为可读写状态
                            field.setAccessible(true);
                            //字段值
                            Object o = field.get(selecthntbhzone);
                            if (o == null) {
                                continue;
                            }
                            String value = o.toString();
                            //判断
                            if (value.contains("临建") || value.contains("硬化") || value.contains("砂浆")) {
                                status = true;
                                ...
                                bhzCementBase.setShebeiNo(selecthntbhzone.getShebeiNo() + "yc");
                ...
                if (selectcementlist.size() == 0) {//如果为空呢 就把此数据更改为异常状态
                    bhzCementBaseService.updatehntbhzone(batchNo, 30);
                    BhzCementBase bhzCementBase = new BhzCementBase();
                    bhzCementBase.setId(selecthntbhzone.getId());
