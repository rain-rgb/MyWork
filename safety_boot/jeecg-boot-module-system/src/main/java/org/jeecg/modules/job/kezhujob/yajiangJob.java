...
public class yajiangJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.KZ_YJ);//柯诸压浆
        //如果他等于空
        ...
        List<TrYajiangM> trYajiangMS = yajiangMService.selectyjList(shebeilist, selectsysconfigone.getCurid());
        ...
        for (TrYajiangM trYajiangM : trYajiangMS) {
            ...
            for (TrYajiangS trYajiangS : yajiangSList) {
                ...
                if (StringUtil.isNotEmpty(chixushijia)){
                    //chixushijia 去除所含字母
                    chixushijia = chixushijia.replaceAll("[^0-9]", "");
                    int chixuSeconds = Integer.parseInt(chixushijia);
                ...
                sysConfigService.updateSysConfig(JobUtil.KZ_YJ, id);
            }
        }
    }
