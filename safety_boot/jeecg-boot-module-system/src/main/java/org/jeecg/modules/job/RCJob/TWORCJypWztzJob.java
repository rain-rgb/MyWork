...
public class TWORCJypWztzJob implements Job {
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        // 地磅类型检验批推送
        List<JypWztz> jypWztzs = wztaizhangService.queryJypList2(selectsysconfigone.getCurdate());//查询jystatus=0，isfinish=1的数据进行推送
        ...
        for (JypWztz jypWztz : jypWztzs) {
            ...
            if(com.trtm.api.utils.StringUtils.isNotBlank(jypWztz.getLcbh())){
            ...
            if (!StringUtils.isEmpty(jypWztz.getPici()) && jypWztz.getPici().length() > 3 && "JYP".equals(jypWztz.getPici().substring(0, 3).toUpperCase())) {
                jsonObject1.set("shengchanpihao", "/");
            } else {
                jsonObject1.set("shengchanpihao", jypWztz.getPici());
            }
//            jsonObject1.set("shengchanpihao",jypWztz.getShengchanpihao());
            ...
            if ("0".equals(success)) {
                ...
                wztaizhangService.update(new UpdateWrapper<Wztaizhang>().lambda().set(Wztaizhang::getJystatus, 2).eq(Wztaizhang::getId, jypWztz.getId()));//推送失败，修改jystatus值为2
                sysConfigService.updateSysConfig(JobUtil.RC_JypWztz, jypWztz.getId());
            }
        }
    }
