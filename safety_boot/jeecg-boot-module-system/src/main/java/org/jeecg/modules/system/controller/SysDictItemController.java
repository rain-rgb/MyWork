...
public class SysDictItemController {
	...
	@AutoLog(value = "字典数据-理论配合比材料名")
	@ApiOperation(value="字典数据-理论配合比材料名", notes="字典数据-理论配合比材料名")
	@GetMapping(value = "/cailiaonamelist")
	public Result<?> queryPageList1(SysDictItem sysDictItem, HttpServletRequest req) {
		String dictCode = "material_types";
		SysDict selectdict = sysDictService.selectdict(dictCode);
		SysDictItem selectdictid = sysDictItemService.selectdictid(sysDictItem.getItemValue(),selectdict.getId());
		SysDict selectdict1 = sysDictService.selectdict(selectdictid.getDescription());
		List<SysDictItem> selectcailiaoname = sysDictItemService.selectcailiaoname(selectdict1.getId());
		return Result.OK(selectcailiaoname);
	}
