...
public class RCzhydhisJob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.RCZHYL_HIS);//瑞仓智慧用电历史数据
        //如果他等于空
        if (null == selectsysconfigone) {
            log.info(String.format("未获取到瑞仓智慧用电历史数据定时任务的配置信息" + DateUtils.now()));
            return;
        }
        //当前数据检测到的目标id
        ...
        if (null == jsonObject || jsonObject.isEmpty()) {
            log.info(String.format("没有配置要传输瑞仓智慧用电历史数据的设备" + DateUtils.now()));
            return;
        }
        String shebeilist = jsonObject.getStr("shebeilist");
        List<DeviceElectricHistorydata> list = deviceElectricHistorydataService.selectHisList(curid,shebeilist);
        if (null == list || list.size() == 0) {
            log.info(String.format("暂无瑞仓智慧用电历史未推送数据" + DateUtils.now()));
            return;
        }
        int id = 0;
        for (DeviceElectricHistorydata deviceElectricHistorydata : list){
            ...
            if (code == 200) {
                log.info(String.format("瑞仓智慧用电历史数据推送成功!" + id));
                sysConfigService.updateSysConfig(JobUtil.RCZHYL_HIS, id);//根据传过来的唯一值来修改当前判断到的数据id
            }else if(code == 402){
                log.info(String.format("瑞仓智慧用电历史数据推送失败，数据重复，更新配置信息!" + id));
                sysConfigService.updateSysConfig(JobUtil.RCZHYL_HIS, id);//根据传过来的唯一值来修改当前判断到的数据id
            } else {
                log.info(String.format("瑞仓智慧用电历史数据推送失败!" + id));
            }
        }
    }
}
