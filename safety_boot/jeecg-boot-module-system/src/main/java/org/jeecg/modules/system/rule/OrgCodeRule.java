...
public class OrgCodeRule implements IFillRuleHandler {
    ...
    public Object executes(JSONObject params, JSONObject formData) {
        ISysDepartprojectService sysDepartService = (ISysDepartprojectService) SpringContextUtils.getBean("sysDepartprojectServiceImpl");
        ...
        if (StringUtil.isNullOrEmpty(parentId)) {
            ...
            // 查询出同级部门的集合
            List<SysDepartproject> parentList = sysDepartService.list(query);
            // 查询出父级部门
            SysDepartproject depart = sysDepartService.getById(parentId);
            // 获取父级部门的Code
            String parentCode = depart.getOrgCode();
            // 根据父级部门类型算出当前部门的类型
            orgType = String.valueOf(Integer.valueOf(depart.getOrgType()) + 1);
