 ...
 * @author xgd
 * @date 2021/3/17
 * 砼拌合站超标检测定时任务(用来处理历史数据问题)
 */
@Slf4j
@PersistJobDataAfterExecution
@DisallowConcurrentExecution
public class TbhzJob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.HNTBHZ_CFG);//拌合站超标监测=4
        //如果他等于空
        ...
        for (BhzCementBase selecthntbhzone : selecthntbhzones) {
            if ("jcebbh01".equals(selecthntbhzone.getShebeiNo())&&!selecthntbhzone.getProjectName().contains("交工")){
                selecthntbhzone.setShebeiNo("jcebbh01sc");
            ...
            //先修改判断位状态
            BhzCementBase bhzCementBases=new BhzCementBase();
            bhzCementBases.setId(selecthntbhzone.getId());
            bhzCementBases.setAlertstate(1);
            bhzCementBaseService.updateById(bhzCementBases);
...
//                boolean b = true;
//                if (level == 1){
//                    assert selectbhzcallone != null;
//                    b = DySmsHelper.sendSms(selectbhzcallone.getPrimaryPhones(), obj, DySmsEnum.SMS_CB_CODE);
//                }else if (level == 2){
//                    assert selectbhzcallone != null;
//                    b = DySmsHelper.sendSms(selectbhzcallone.getMiddlePhones(), obj, DySmsEnum.SMS_CB_CODE);
//                }else if (level == 3){
//                    assert selectbhzcallone != null;
//                    b = DySmsHelper.sendSms(selectbhzcallone.getAdvancedPhones(), obj, DySmsEnum.SMS_CB_CODE);
//                }
//                if (b){
...
//
        ...
        sysConfigService.updateSysConfig(JobUtil.HNTBHZ_CFG, id);//根据传过来的唯一值来修改当前判断到的数据id
    }
}
