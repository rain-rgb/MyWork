package org.jeecg.modules.job.sutaiTwoThreejob;
...
@Slf4j
public class STHntbhzJob implements Job {
    @Autowired
    private ISysConfigService sysConfigService;
    @Autowired
    private IShebeiInfoService shebeiInfoService;
    @Autowired
    private SuTaiUtil suTaiUtil;
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.STTWO_THREEHNTBHZ);//苏台二期三标混凝土拌合站
        //如果他等于空
...
//            saveDTOList.set("equipmentName", "苏台二期三标测试拌合站");//设备名称
            ...
            saveDTOList.set("sectionType", 3);//1 一标 2 二标 3 三标 4 四标 5 五标
        ...
        if (jsonArray.size() > 0) {
            Integer integer = suTaiUtil.PostSTTHIBHZList(jsonArray);
            if (integer == 200) {
                log.info(String.format("苏台拌合站二期三标数据推送成功!" + id));
                sysConfigService.updateSysConfig(JobUtil.STTWO_THREEHNTBHZ, id);//根据传过来的唯一值来修改当前判断到的数据id
            } else {
                log.info(String.format("苏台拌合站二期三标数据推送失败!" + id));
            }
        } else {
            log.info(String.format("苏台拌合站当前数据属于二期三标" + id));
            sysConfigService.updateSysConfig(JobUtil.STTWO_THREEHNTBHZ, id);//根据传过来的唯一值来修改当前判断到的数据id
        }
    }
}
