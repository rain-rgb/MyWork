...
@Slf4j
public class SyjTuiSongJob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        //根据定时任务配置表获取一条配置信息
        SysConfig selectsysconfigone = sysConfigService.selectsysconfigone(JobUtil.SYJ_TUISONG);
        //如果他等于空
        ...
        for (TSyjzb syjOne : syjList) {
            ...
            if (isHave(sylxWnjList, sylx)) {
                ...
                String back = HttpRequest.post("http://z.traiot.cn/jeecg-boot/syj/tSyjzb/addWNJOpen")
                ...
                if (code == 200) {
                    log.info(String.format("试验万能机数据推送成功" + DateUtils.now()));
                    sysConfigService.updateSysConfig(JobUtil.SYJ_TUISONG, id);//根据传过来的唯一值来修改当前判断到的数据id
                } else {
                    log.info(String.format("试验万能机数据推送失败" + DateUtils.now()));
                }
            }
            if (isHave(sylxYljList, sylx)) {
                ...
                String back = HttpRequest.post("http://z.traiot.cn/jeecg-boot/syj/tSyjzb/yljadd")
                        .body(String.valueOf(object))
                        .execute()
                        .body();
                JSONObject jsonObject2 = new JSONObject(back);
                Integer code = (Integer) jsonObject2.get("code");
                if (code == 200) {
                    log.info(String.format("试验压力机数据推送成功" + DateUtils.now()));
                    sysConfigService.updateSysConfig(JobUtil.SYJ_TUISONG, id);//根据传过来的唯一值来修改当前判断到的数据id
                } else {
                    log.info(String.format("试验压力机数据推送失败" + DateUtils.now()));
                }
            }
        }
    }
