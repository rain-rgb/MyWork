import Vue from 'vue'
import { login, logout, phoneLogin, thirdLogin, loginsso, loginsso12 } from "@/api/login"
import { ACCESS_TOKEN, USER_NAME,USER_INFO,USER_AUTH,SYS_BUTTON_AUTH,UI_CACHE_DB_DICT_DATA,TENANT_ID,CACHE_INCLUDED_ROUTES,SYS_DEPART_ORGCODE,SAFETY_PARTIAL_BASIC } from "@/store/mutation-types"
...
const user = {
  ...
  mutations: {
    ...
    SYS_DEPART_ORGCODE:(state, token)=>{
    Vue.ls.set(SYS_DEPART_ORGCODE,token)
    },
    SAFETY_PARTIAL_BASIC:(state, token)=>{
      state.sys_depart_project=token;
    },
  ...
  actions: {
      ...
      return new Promise((resolve, reject) => {
        login(userInfo).then(response => {
          if(response.code =='200'){
            ...
            Vue.ls.set('SYS_DEPART_ORGCODE', userInfo.orgCode, 7 * 24 * 60 * 60 * 1000)//把组织机构加入缓存
            commit('SET_TOKEN', result.token)
            commit('SET_INFO', userInfo)
            commit('SET_NAME', { username: userInfo.username,realname: userInfo.realname, welcome: welcome() })
            commit('SET_AVATAR', userInfo.avatar)
    ...
    Loginsso({ commit }, userInfo) {
      return new Promise((resolve, reject) => {
        loginsso(userInfo).then(response => {
          if(response.code =='200'){
            const result = response.result
            const userInfo = result.userInfo
            Vue.ls.set(ACCESS_TOKEN, result.token, 7 * 24 * 60 * 60 * 1000)
    ...
    //单点连接登录
    Loginsso12({ commit }, userInfo) {
      return new Promise((resolve, reject) => {
        loginsso12(userInfo).then(response => {
          if(response.code =='200'){
            const result = response.result
            const userInfo = result.userInfo
            Vue.ls.set(ACCESS_TOKEN, result.token, 7 * 24 * 60 * 60 * 1000)
            ...
            commit('SET_NAME', { username: userInfo.username,realname: userInfo.realname, welcome: welcome() })
            commit('SET_AVATAR', userInfo.avatar)
            commit('SYS_DEPART_ORGCODE', userInfo.orgCode)
            resolve(response)
          }else{
            reject(response)
          }
