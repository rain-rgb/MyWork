<template>
  <div class="components-input-demo-presuffix">
    <!---->
    <a-input @click="openModal" placeholder="请点击选择施工部位" v-model="departNames" readOnly :disabled="disabled">
      <a-icon slot="prefix" type="cluster" title="施工部位选择控件"/>
      <a-icon v-if="departIds" slot="suffix" type="close-circle" @click="handleEmpty" title="清空"/>
    </a-input>

    <JselectDqProjNameModal
...
<script>
import JselectDqProjNameModal from './modal/JselectDqProjNameModal'
export default {
  name: 'JSelectDqProjName',
  components:{
    JselectDqProjNameModal
  },
  props:{
  ...
  mounted(){
    ...
    console.log( this.departIds,"33333",this.value)
  },
  watch:{
    value(val){
      console.log(val,"4444444444")
  ...
  methods:{
    initComp(departNames){
      ...
      //TODO 当返回字段为工程名称时会有问题,因为工程名称不唯一
    ...
    handleOK(rows, idstr) {
      let value = ''
      console.log(rows,idstr,"当前选择")
      if (!rows && rows.length <= 0) {
        this.departNames = ''
        this.departIds = ''
      } else {
        value = rows.map(row => row[this.customReturnField]).join(',')
        this.departNames = rows.map(row => row['description']).join(',')
