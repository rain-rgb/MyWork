...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      /** 判断某个ID是否包含了caseId */
      hasCaseId(id) {//使用线上开发的时候需要打开的下面禁用内容
        // return id && id.startsWith(this.caseId)
        return id
      },

      /** 将caseId去除 */
      removeCaseId(id) {//使用线上开发的时候需要打开的下面禁用内容
       // if (this.hasCaseId(id)) {
      ...
      // 判断 id 是否是临时Id
      isTempId(id) {//使用线上开发的时候需要打开的下面禁用内容
        // return (id || '').endsWith(this.tempId)
        return id
      },

      /** 将tempId去除 */
      removeTempId(id) {//使用线上开发的时候需要打开的下面禁用内容
      // if (this.isTempId(id)) {
