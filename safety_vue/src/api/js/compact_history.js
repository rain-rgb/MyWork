function compackWord() {
  ...
  showList.forEach(function(rollerId) {
    if (allTrackData[rollerId] == undefined) {
      return;
    }
    let ptsData = allTrackData[rollerId];
    let index = calcWorkIndex(allInfoData[rollerId]);
    let count = Math.min((index+1) * 2, ptsData.length)
    drawCompackBlock(ctx, ptsData.slice(0, count));
  })
...
function calTrackPtsByWorker(pts, rollerId) {
  ...
  if (typeof Worker !== "undefined") {
    ...
    calTrackWorker.onmessage = function(e) {
      var rollerId = e.data.rollerId;
      if (allTrackData[rollerId] != undefined) {
        allTrackData[rollerId] = allTrackData[rollerId].concat(e.data.pts);
      } else {
        allTrackData[rollerId] = e.data.pts;
      }
    }
  } else {
