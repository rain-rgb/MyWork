...
/**
 * 获取特定日期
 * @param {number|string} days - 相对天数（如1表示1天后）或特定日期字符串（如'2025-05-08'）
 * @returns {string} 返回YYYY-MM-DD格式的日期字符串
 */
export function getSpecificDate(days) {
  const date = new Date();
  ...
  // 如果是字符串，尝试解析为特定日期
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(days)) {
      return days; // 直接返回格式化的日期字符串
    }
    // 如果不是标准格式，尝试解析
    const parsedDate = new Date(days);
