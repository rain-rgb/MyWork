...
export default {
  ...
  data() {
    return {
      ...
      url: {
        ...
        sbbhtime:"/wbshebeidetail/wbshebeiDetail/list",//电子锁轨迹-设备编号和时间
      },
    };
  },
  computed: {
  ...
  methods: {
    ...
    detail(record) {
      ...
      this.visible = true;
      this.sbbhtime(record);
    },
    Query() {
      //搜索
      console.log(this.datatime_begin,'this.datatime_begin-----------',);
      if (this.$Jlodash.isEmpty(this.datatime_begin)) {
        this.$message.warning("请选择开始时间");
        return;
      }
      if (this.$Jlodash.isEmpty(this.datatime_end)) {
    ...
    sbbhtime(record){
      ...
      getAction(this.url.sbbhtime, params).then((res) => {
        if (res.success) {
          let data = res.result.records[0];
          if(res.result.records.length > 0){
            this.$set(this.formList, 'sbbh', data.sbbh);
            this.$set(this.formList, 'cftime', data.cftime);
            this.$set(this.formList, 'ddtime', data.ddtime);
            console.log(this.formList,'records--------------',this.form);
          }else{
            this.$message.warning("未匹配 对应电子锁运输车!");
          }
    ...
    Carguiji: function (record) {
      ...
      if (!Boolean(this.formList.ddtime)) {
        //到达时间没有值时获取当前时间
        let currentDate = new Date();
        this.datatime_end = this.formatDate(currentDate);
      } else {
        this.datatime_end = this.formList.ddtime;
      }
      this.datatime_begin = this.formList.cftime;
      this.content = this.formList.sbbh;
      // let params = { shebeiNo: this.shebeiNo, projectId: record.datatime, uploadtime: datatime_start }
      let params = {
        pageSize: 2000,
        pageNum: 1,
        sn: this.formList.sbbh,
        // sn: 867192069426432,//this.listdata.sbbh
        // teid: this.teid,
        start_time: this.formList.cftime,
        end_time: this.formList.ddtime,
      };
      getAction(this.url.tokens, params).then((res) => {
        if (res.success) {
          if (res.result.length > 0) {
