...
<!--          </a-col>-->
    ...
    <a-tabs default-active-key="1" @change="changeTabs">
      <a-tab-pane key="1" tab="进出场影像">
        <viewer :images="detailList">
          <div v-for="(item,index) in detailList" :key="index">
            <img
              v-if="containsHttpOrHttps(item.icon) && containsImageExtension(item.icon)"
              style="height:450px;width: 550px;margin: 5px 10px 5px 10px;float: left"
              :src="[item.icon] "
              alt="">
            <img
              v-else-if="containsImageExtension(item.icon)"
              style="height:450px;width: 550px;margin: 5px 10px 5px 10px;float: left"
      ...
      <a-tab-pane key="2" tab="质检资料" >
        <a @click="handlePreview(fcdmodel)">预览</a>
      </a-tab-pane>
      <a-tab-pane key="3" tab="运输轨迹图">
        <ClxxRealdataModal ref="modalFormC"></ClxxRealdataModal>
      </a-tab-pane>
...
<script>
  ...
  // import ClxxRealdataModal from './ClxxRealdataModal'
  import ClxxRealdataModal from './ClxxRealdataForm'

  export default {
    name: 'WzycljinchanggbForm',
    components: {
      JFormContainer,ClxxRealdataModal,
    },
    props: {
      //流程表单data
      formData: {
    ...
    data () {
      return {
        ...
        url: {
          ...
          queryfcd:"/wbshebeidetail/wbshebeiDetail/list"
        }
      }
    },
    computed: {
    ...
    methods: {
      changeTabs(index){
        if(index == 3){
          this.handleDetail1(this.model);
        }
        console.log(index,this.model,`idnexdalkdnaslk`);
      },
      ...
      handlePreview(record) {
        if (record && record.jyimgfile) {
          let url = window._CONFIG['onlinePreviewDomainURL'] + '?url=' + encodeURIComponent(base64Encode(record.jyimgfile))
      ...
      getFcd:function (){
        let params = {danhao:this.model.houchepai,cph:this.model.qianchepai}
        getAction(this.url.queryfcd,params).then((res)=> {
          if(res.success){
            this.fcdmodel = res.result[1];
          }
        });
      },
      ...
      containsHttpOrHttps(str) {  
        // 使用includes方法检查字符串是否包含http://或https://  
