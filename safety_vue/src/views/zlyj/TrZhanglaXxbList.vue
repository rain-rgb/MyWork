<template>
  <a-card :bordered="false">
    <a-row :gutter="24">
      <a-col :sm="24" :md="12" :xl="4" :style="{ marginBottom: '24px'}"><span style="font-size: 22px;font-weight: bold">张拉未合格：{{oversum}}</span> </a-col>
      <a-col :sm="24" :md="12" :xl="4" :style="{ marginBottom: '24px'}"><span style="font-size: 22px;font-weight: bold">本月未合格：{{monthOverSum}}</span> </a-col>
    </a-row>
    <!-- 查询区域 -->
        ...
        <a-row :gutter="24">
          ...
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="时间范围">
              <j-date placeholder="开始时间" v-model="queryParam.sgsj_begin" :showTime="true"
                      dateFormat="YYYY-MM-DD HH:mm:ss"/>
            </a-form-item>
          </a-col>
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="">
              <j-date placeholder="结束时间" v-model="queryParam.sgsj_end" :showTime="true"
                      dateFormat="YYYY-MM-DD HH:mm:ss"/>
            </a-form-item>
          </a-col>
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
       ...
       <span slot="status" slot-scope="status, record">
        ...
        <span slot="hege" slot-scope="hege, record">
          <a-tag color="red" v-if="record.hege == '0'">不合格</a-tag>
           <a-tag color="green" v-else>合格</a-tag>
        </span>
...
import TrZhanglaMModal from '@views/zlyj/modules/TrZhanglaMModal'
import Vue from 'vue'
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: '/zhanglaxxb/trZhanglaXxb/list',
  ...
  methods: {
    export1:function() {//打印功能需要先去报表设计页面设计打印格式
    ...
    tongji: function () {
      ...
      getAction(this.url.zltjUrl, { sys_depart_orgcode })
        .then(res => {
          var data = res.result
    ...
    showmadel1(syjid) {
      this.$refs.TrZhanglaMModal.title = '详情'
      this.$refs.TrZhanglaMModal.call(syjid)
    },
    shebeiList: function () {
      var sys_depart_orgcode = Vue.ls.get('SYS_DEPART_ORGCODE')
      usershebeiList({
