  ...
  <a-card :bordered="false">
    ...
    <div class="table-page-search-wrapper">
      <a-form layout="inline" @keyup.enter.native="searchQuery">
        <a-row :gutter="24">
          ...
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="梁板类型">
              <a-input placeholder="请输入梁板类型" v-model="queryParam.lblx"></a-input>
            </a-form-item>
          </a-col>
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
          ...
          <a-form-item label="工程部位">
            <a-input placeholder="请输入工程部位" v-model="queryParam.lianghao"></a-input>
          </a-form-item>
        </a-col>
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="超标查询">
              <j-dict-select-tag placeholder="请选择是否超标" v-model="queryParam.isOverLevel"
                                 dictCode="isOverLevel"></j-dict-select-tag>
            </a-form-item>
          </a-col>
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
    ...
    <div>
      ...
      >
        ...
        <a-tag color="green" v-if="record.status == '1'">完成</a-tag>
         <a-tag color="red" v-if="record.status == '0'">进行中</a-tag>
       </span>
        <span slot="isHege" slot-scope="isHege, record">
        <a-tag color="green" v-if="record.isHege == '1'">合格</a-tag>
         <a-tag color="red" v-if="record.isHege == '0'">不合格</a-tag>
       </span>
        <span slot="isOverLevel" slot-scope="isOverLevel, record">
        <a-tag color="red" v-if="record.isOverLevel == '1'">不合格</a-tag>
         <a-tag color="green" v-if="record.isOverLevel == '0'">合格</a-tag>
          <a-tag color="green" v-if="record.isOverLevel == null">合格</a-tag>
       </span>
        <span slot="bindrwd" slot-scope="text, record">
          <a-tag color="grey" v-if="record.bindrwd == null">未关联</a-tag>
          <a-tag color="blue" v-if="record.bindrwd == 10">已关联</a-tag>
          <a-tag color="green" v-if="record.bindrwd == 20">已通过</a-tag>
        </span>
        ...
        <span slot="action" slot-scope="text, record">
          <a v-has="'yj:rwtb'" v-if="record.bindrwd == null" @click="handleEdit1(record)">填报</a>
          <a  v-has="'yj:rwsh'" v-if="record.bindrwd == 10"  @click="handleEdit1(record)">审核</a>
          <a v-if="record.bindrwd == 20"  @click="handleEdit1(record)">填报详情</a>
    ...
    <TBTrYaJiangModal ref="modalForm1" @ok="modalFormOk"></TBTrYaJiangModal>
  </a-card>
</template>
...
import TBTrYaJiangModal from './modules/TBTrYaJiangModal'
  ...
  data() {
    return {
      ...
      columns: [
        ...
        //   align: 'center',
        //   dataIndex: 'isHege',
        //   scopedSlots: { customRender: 'isHege' },
        // },
        {
          title: '是否合格',
          align: 'center',
        ...
        },
        {
          ...
          align: 'center',
          dataIndex: 'bindrwd',
          scopedSlots: { customRender: 'bindrwd' },
        },
        {
          title: '操作',
          dataIndex: 'action',
      ...
      url: {
        list: '/yajiangm/trYajiangM/list47NoUse',
