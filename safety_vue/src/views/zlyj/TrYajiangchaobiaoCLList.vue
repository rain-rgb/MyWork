<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="24">
          ...
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="时间范围">
              <j-date placeholder="开始时间" v-model="queryParam.yajiangsj_begin" :showTime="true"
                      dateFormat="YYYY-MM-DD HH:mm:ss"/>
            </a-form-item>
          </a-col>
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="">
              <j-date placeholder="结束时间" v-model="queryParam.yajiangsj_end" :showTime="true"
                      dateFormat="YYYY-MM-DD HH:mm:ss"/>
            </a-form-item>
          </a-col>
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
    ...
    <div>
      ...
      >
        ...
        <span class="separate" slot="kongdaohao" slot-scope="text,record">
          <div v-for="(item,index) in record.yajiangs">
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <a-dropdown>
            ...
            <a-menu slot="overlay">
             <a-menu-item v-has="'yajiang:cz'" v-if="record.zhanglaYajiangOverHandler.overproofStatus == '0'||record.zhanglaYajiangOverHandler.overproofStatus == '30'">
                <a @click="showmadelchuzhi(record.syjid,2)">处置</a>
              </a-menu-item>
              <a-menu-item v-has="'yajiang:sh'">
                <a @click="showmadelshenhe(record.syjid,2)" v-if="record.zhanglaYajiangOverHandler.overproofStatus == '10'">审核</a>
              </a-menu-item>
              <a-menu-item v-has="'yajiang:bh'" v-if="record.zhanglaYajiangOverHandler.overproofStatus == '10'">
                <a @click="showmadelbohui(record.syjid,2)">驳回</a>
              </a-menu-item>
            </a-menu>
          </a-dropdown>
        </span>
    ...
    <BoHui ref="bohui"></BoHui>
    <ShenHe ref="shenhe"></ShenHe>
    <ChuZhi ref="chuzhi"></ChuZhi>
    <TrYajiangMSModalChuZhiShenHe ref="TrYajiangMSModal"></TrYajiangMSModalChuZhiShenHe>
...
import TrYajiangMSModalChuZhiShenHe from '@views/zlyj/modules/TrYajiangMSModalChuZhiShenHe'
import Vue from 'vue'
import ChuZhi from '@views/zlyj/czsh/ChuZhi'
import ShenHe from '@views/zlyj/czsh/ShenHe'
import BoHui from '@views/zlyj/czsh/BoHui'
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: '/yajiangs/trYajiangS/list2',
  ...
  methods: {
    showmadelchuzhi(syjid,i){
      this.$refs.chuzhi.showModal(syjid,i);
    },
    showmadelshenhe(syjid,i){
      this.$refs.shenhe.showModal(syjid,i);
    },
    showmadelbohui(syjid,i){
      this.$refs.bohui.showModal(syjid,i);
    },
    showmadel1(syjid) {
