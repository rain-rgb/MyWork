<template>
  <a-card :bordered="false">
      <a-row :gutter="24">
      <a-col :sm="24" :md="12" :xl="4" :style="{ marginBottom: '24px' }"><span style="font-size: 22px;font-weight: bold">压力机预警：{{yljALlWarn}}</span></a-col>
      <a-col :sm="24" :md="12" :xl="4" :style="{ marginBottom: '24px' }"><span style="font-size: 22px;font-weight: bold">本月预警：{{yljMonthWarn}}</span></a-col>
    </a-row>
    <!-- 查询区域 -->
    ...
    <SyyLjBasesModal ref="modalForm" @ok="modalFormOk"/>
  </a-card>
</template>
  ...
  data() {
    return {
      ...
      url: {
        ...
        syjwarn:"/tsyjzbStatistics/tSyjzbStatistics/yljwarn"
      },
      dictOptions: {},
      superFieldList: [],
    }
    ...
    changecjmc:function () {
     ...
     tongji:function(){
        ...
        getAction(this.url.syjwarn,{sys_depart_orgcode})
          .then(res=>{
            var data = res.result
            this.yljMonthWarn = data.mwarn;
            this.yljALlWarn = data.allWarn;
          })
      },
    shebeiList: function () {
      var sys_depart_orgcode = Vue.ls.get('SYS_DEPART_ORGCODE')
