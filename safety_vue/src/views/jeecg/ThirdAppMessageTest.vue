          ...
          <a-tab-pane tab="消息选项" key="1">
            ...
            >
              <a-switch
                checkedChildren="是"
                unCheckedChildren="否"
                v-model="model.sendAll"
                @change="onSendAllChange"
              />
            </a-form-model-item>
            <a-form-model-item
              label="接收人"
            ...
            <div style="text-align: center">
              <a-button type="primary" size="large" @click="onSend" style="width: 120px"
                >发送</a-button
              >
            </div>
          </a-tab-pane>
  ...
  methods: {
    ...
    onSend() {
      this.$refs.form.validate((ok, err) => {
        if (ok) {
          this.loading = true;
          postAction("/sys/thirdApp/sendMessageTest", this.model)
            .then(({ success, result, message }) => {
              if (success) {
                ...
                this.dataSource.unshift(
                  Object.assign({ id: randomUUID() }, this.model, { response })
                );
              } else {
