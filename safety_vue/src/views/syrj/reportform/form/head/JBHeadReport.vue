    ...
    <div class="head">
        ...
        <div class="tablecontent">
            <table class="table-noborder">
                <tr>
                    <td style="width: 86px">试验室名称：</td>
                    <td style="height: 28px">{{ reportData.gongZhangMingCheng }}</td>
                    <td style="width: 230px; white-space: nowrap">
                        &emsp;&emsp;&emsp;报告编号：
                        <span class="tableNumber">{{ reportData.reportNo }}</span>
                    </td>
                </tr>
            </table>
            <table class="la-table" style="width: 100%">
                <tr>
                    <td style="width: 13%">施工/委托单位</td>
                    <td style="width: 39%"><a-textarea v-model="reportData.shigongdanwei" auto-size /></td>
                    <td style="width: 9%">工程名称</td>
                    <td style="width: 39%"><a-textarea v-model="reportData.gongchengmingcheng" auto-size /></td>
                </tr>
                <tr>
                    <td>工程部位/用途</td>
                    <td colspan="3"><a-textarea v-model="reportData.sampleGcbw" auto-size /></td>
                </tr>
                <tr>
                    <td>委托/任务编号</td>
                    ...
                    <td>
                        <a-range-picker @change="onChange">
                            <span style="color: #000; font-size: 12px">
                                {{ reportData.baogaoriqi ? reportData.baogaoriqi : '请选择日期' }}
                            </span>
                        </a-range-picker>
                    </td>
                </tr>
    ...
    methods: {
        ...
        onChange(time) {
            this.timestampToTime(time[0]) == this.timestampToTime(time[1])
                ? (this.reportData.baogaoriqi = this.timestampToTime(time[0]))
                : (this.reportData.baogaoriqi = this.timestampToTime(time[0]) + ' 至 ' + this.timestampToTime(time[1]))
        },
    },
}
</script>
