<template>
  <a-spin :spinning="confirmLoading">
          ...
          <a-row>
            <a-col :span="12">
              <a-form-item label="混合料类型" :labelCol="labelCol" :wrapperCol="wrapperCol">
                <j-dict-select-tag type="list" v-decorator="['hhllx']" :trigger-change="true" dictCode="swhhl"
          ...
          <a-row>
            <a-col :span="12">
              <a-form-item label="配比名称" :labelCol="labelCol" :wrapperCol="wrapperCol">
                <j-dict-select-tag type="list" v-decorator="['lilunname']" :trigger-change="true" dictCode="swlilunname"
                                   placeholder="请选择配比名称"/>
              </a-form-item>
            </a-col>
            <a-col :span="12">
              <a-form-item label="施工部位" :labelCol="labelCol" :wrapperCol="wrapperCol">
                <j-dict-select-tag type="list" v-decorator="['llbuwei']" :trigger-change="true" dictCode="swllbuwei"
                                   placeholder="请选择施工部位"/>
              </a-form-item>
            </a-col>
          </a-row>
    ...
    <!-- 子表单区域 -->
    <a-tabs>
      <a-tab-pane tab="水稳拌合站理论配合比子表" :key="refKeys[0]" :forceRender="true">
        <j-editable-table
          :ref="refKeys[0]"
          :loading="bhzSwPhbZibiaoTable.loading"
          :columns="bhzSwPhbZibiaoTable.columns"
          :dataSource="bhzSwPhbZibiaoTable.dataSource"
          :maxHeight="300"
  ...
  export default {
    ...
    data() {
      return {
        ...
        // 新增时子表默认添加几行空数据
        addDefaultRowNum: 0,
        refKeys: ['bhzSwPhbZibiao',],
        tableKeys: ['bhzSwPhbZibiao',],
        activeKey: 'bhzSwPhbZibiao',
        // 水稳拌合站理论配合比子表
        bhzSwPhbZibiaoTable: {
          loading: false,
          dataSource: [],
        ...
        url: {
          ...
          bhzSwPhbZibiao: {
            list: '/bhzSwPhbZibiao/bhzSwPhbZibiao/list1'
          },
        }
      }
    },
    ...
    methods: {
      ...
      addBefore() {
        ...
        this.bhzSwPhbZibiaoTable.dataSource = []
      },
      getAllTable() {
      ...
      editAfter() {
        ...
        if (this.model.zhuziid) {
          ...
          this.requestSubTableData(this.url.bhzSwPhbZibiao.list, params, this.bhzSwPhbZibiaoTable)
        }
      ...
      classifyIntoFormData(allValues) {
        ...
        return {
          ...main, // 展开
          bhzSwPhbZibiaoList: allValues.tablesValue[0].values,
        }
      },
      //渲染流程表单数据
      showFlowData() {
