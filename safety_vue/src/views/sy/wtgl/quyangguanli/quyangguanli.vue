        ...
        <a-row :gutter="24">
          ...
          <a-col :xl="5" :lg="7" :md="8" :sm="24">
            <a-form-item label="指派状态">
              ...
              <a-radio-group name="radioGroup" :default-value="1" v-model="queryParam.zhipaizhuangtai">
                <a-radio :value="1">全部</a-radio>
                <a-radio :value="2">未指派</a-radio>
                <a-radio :value="3">已指派</a-radio>
              </a-radio-group>
            </a-form-item>
          </a-col>
        </a-row>
        <a-row>
          <a-col span="21">
            <span class="table-page-search-submitButtons">
              <a-button @click="zhipai" type="primary">指派</a-button>
              <a-button @click="bianji" type="primary" style="margin-left: 8px">编辑</a-button>
            </span>
          </a-col>
          <a-col span="3">
            <span class="table-page-search-submitButtons">
...
<!--    <div>
      ...
      <a-table
        ...
        <span slot="tags" slot-scope="tags, record">
          <a-tag color="red" v-if="record.delegateState == 0">未委托</a-tag>
          <a-tag color="green" v-if="record.delegateState == 1">已委托</a-tag>
        </span>
        <span slot="tags1" slot-scope="tags1, record">
          <a-tag color="red" v-if="record.zhipaizhuangtai == 0">未指派</a-tag>
          <a-tag color="green" v-if="record.zhipaizhuangtai == 1">已指派</a-tag>
        </span>
    ...
    <sy-model ref="sy"></sy-model>
    <wei-tuo-model ref="wt"></wei-tuo-model>
  </a-card>
</template>
...
import Vue from 'vue'
import SyModel from './modules/syModel'
import WeiTuoModel from './modules/weiTuoModel'
  ...
  methods: {
    zhipai() {
      if (this.selectionRows.length == 1) {
        this.$refs.sy.visible = true
        this.$refs.sy.getUser()
      } else {
    ...
    bianji() {
      if (this.selectionRows.length == 1) {
        this.$refs.wt.visible = true
        this.$refs.wt.model = this.selectionRows[0]
      } else {
        this.$message.error('请选择单行数据！')
      }
    },
