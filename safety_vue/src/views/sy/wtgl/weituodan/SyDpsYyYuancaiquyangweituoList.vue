...
<!--            <a-form-item label="材料厂家">
              <a-input placeholder="" v-model="queryParam.gongyingshangdanweibianma"></a-input>
            </a-form-item>-->
            <a-form-item label="材料厂家">
...
<!--    <div>
      ...
      <a-table
        ...
        <span slot="tags" slot-scope="tags, record">
          <a-tag color="green" v-if="record.weituozhuangtai == 0">未委托</a-tag>
          <a-tag color="orange" v-if="record.weituozhuangtai == 1">已委托</a-tag>
        </span>
        <span slot="tags1" slot-scope="tags1, record">
          <a-tag color="green" v-if="record.quyangzhuangtai == 0">未取样</a-tag>
          <a-tag color="orange" v-if="record.quyangzhuangtai == 1">已取样</a-tag>
        </span>
    ...
    <sy-dps-yy-yuancaiquyangweituo-modal ref="modalForm" @ok="modalFormOk"></sy-dps-yy-yuancaiquyangweituo-modal>
  </a-card>
</template>
...
import SyDpsYyYuancaiquyangweituoModal from './modules/SyDpsYyYuancaiquyangweituoModal'
import SampleType from '@/views/sy/dpssysample/modules/SampleType'
import Vue from 'vue'
import { getAction, deleteAction } from '@api/manage'

export default {
  name: 'SyDpsYyYuancaiquyangweituoList',
  mixins: [JeecgListMixin, mixinDevice],
  components: {
    SampleType,
    SyDpsYyYuancaiquyangweituoModal
  },
  data() {
    return {
      selectValue: '',
      ...
      url: {
        list: '/qywtd/syDpsYyYuancaiquyangweituo/selectList',
        delete: '/qywtd/syDpsYyYuancaiquyangweituo/delete',
        deleteBatch: '/qywtd/syDpsYyYuancaiquyangweituo/deleteBatch',
        exportXlsUrl: '/qywtd/syDpsYyYuancaiquyangweituo/exportXls',
        importExcelUrl: '/qywtd/syDpsYyYuancaiquyangweituo/importExcel',
  ...
  methods: {
    ...
    getSuperFieldList() {
      ...
      fieldList.push({ type: 'int', value: 'yuancaijinchangdengjiid', text: '外键，原材进场登记表ID', dictCode: '' })
      fieldList.push({ type: 'int', value: 'quyangzhuangtai', text: '取样状态（0：未取样  1：已取样）默认0', dictCode: '' })
      ...
      fieldList.push({
        type: 'string',
        value: 'quyangren',
        text: '外键，取样人，获取当前登录用户外键T_S_Base_User表的UserName字段',
        dictCode: ''
      })
      fieldList.push({ type: 'string', value: 'quyangshijian', text: '取样时间（YYYY-MM-dd HH:mm:ss）' })
      fieldList.push({ type: 'string', value: 'quyangshijian', text: '取样时间（YYYY-MM-dd HH:mm:ss）', dictCode: '' })
      fieldList.push({ type: 'string', value: 'shouyangren', text: '外键，收样人，获取当前登录用户外键T_S_Base_User表的UserName字段' })
      fieldList.push({
        type: 'string',
        value: 'shouyangren',
        text: '外键，收样人，获取当前登录用户外键T_S_Base_User表的UserName字段',
        dictCode: ''
      })
      fieldList.push({ type: 'string', value: 'shouyangshijian', text: '收样时间（YYYY-MM-dd HH:mm:ss）', dictCode: '' })
      fieldList.push({ type: 'string', value: 'shouyangshijian', text: '收样时间（YYYY-MM-dd HH:mm:ss）' })
      fieldList.push({
        type: 'int',
        value: 'shouyangzhuangtai',
        text: '收样状态（0：未收样  1：已收样 2：已提交 3：在检）默认0',
        dictCode: ''
      })
      ...
      fieldList.push({
        type: 'string',
        value: 'jianzhengren',
        text: '外键，见证人，获取当前登录用户外键T_S_Base_User表的UserName字段',
        dictCode: ''
      })
      ...
      fieldList.push({ type: 'string', value: 'jianzhengshijian', text: '见证时间（YYYY-MM-dd HH:mm:ss）', dictCode: '' })
      fieldList.push({ type: 'int', value: 'jianzhengzhuangtai', text: '见证状态（0：未见证  1：已见证）默认0', dictCode: '' })
      fieldList.push({ type: 'int', value: 'jianzhengzhuangtai', text: '见证状态（0：未见证  1：已见证）默认0' })
      fieldList.push({ type: 'string', value: 'weituodanbianhao', text: '委托单编号，唯一', dictCode: '' })
      fieldList.push({ type: 'string', value: 'weituodanbianhao', text: '委托单编号，唯一' })
      fieldList.push({ type: 'Text', value: 'yangpinchulifangshi', text: '样品处理方式', dictCode: '' })
      fieldList.push({ type: 'string', value: 'yangpinchulifangshi', text: '样品处理方式' })
      fieldList.push({ type: 'string', value: 'quyangdidian', text: '取样地点' })
      fieldList.push({ type: 'Text', value: 'quyangdidian', text: '取样地点', dictCode: '' })
      fieldList.push({ type: 'int', value: 'baogaofenshu', text: '报告份数', dictCode: '' })
      fieldList.push({ type: 'int', value: 'baogaofenshu', text: '报告份数' })
      fieldList.push({ type: 'Text', value: 'jiancexiangmu', text: '检测项目', dictCode: '' })
      fieldList.push({ type: 'string', value: 'jiancexiangmu', text: '检测项目' })
      fieldList.push({ type: 'string', value: 'weituoren', text: '委托人' })
      fieldList.push({ type: 'string', value: 'weituoren', text: '委托人', dictCode: '' })
      fieldList.push({ type: 'string', value: 'weituoriqi', text: '委托日期' })
      fieldList.push({ type: 'string', value: 'weituoriqi', text: '委托日期', dictCode: '' })
      fieldList.push({ type: 'Text', value: 'shigongdanwei', text: '施工单位', dictCode: '' })
      fieldList.push({ type: 'string', value: 'shigongdanwei', text: '施工单位' })
      fieldList.push({ type: 'Text', value: 'jianglidanwei', text: '监理单位', dictCode: '' })
      fieldList.push({ type: 'string', value: 'jianglidanwei', text: '监理单位' })
      fieldList.push({ type: 'Text', value: 'shigongdanweihetonghao', text: '施工单位合同号', dictCode: '' })
      ...
      fieldList.push({ type: 'Text', value: 'jianlidanweihetonghao', text: '监理单位合同号', dictCode: '' })
      fieldList.push({ type: 'string', value: 'jianlidanweidepartid', text: '外键，监理单位组织机构id', dictCode: '' })
      ...
      fieldList.push({ type: 'string', value: 'jianlidanweiorgcode', text: '外键，监理单位组织机构编码', dictCode: '' })
      fieldList.push({
        type: 'string',
      ...
      fieldList.push({ type: 'int', value: 'zhixingzhuangtai', text: '执行状态（0：未执行 1：执行中 2：已完成）', dictCode: '' })
      ...
      fieldList.push({ type: 'Text', value: 'sampleno', text: '外键，样品编号', dictCode: '' })
      fieldList.push({ type: 'string', value: 'erweimabianhao', text: '二维码编号(处理后字段，id+序号)', dictCode: '' })
      fieldList.push({ type: 'string', value: 'erweimabianhao', text: '二维码编号(处理后字段，id+序号)' })
      fieldList.push({ type: 'string', value: 'erweimaweiyima', text: '二维码UUID', dictCode: '' })
      ...
      fieldList.push({
        type: 'int',
        value: 'weituozhuangtai',
        text: '委托状态（0：未提交 1：已提交）默认0，已提交时需更新原材登记表的委托状态为已委托',
        dictCode: ''
      })
      fieldList.push({ type: 'string', value: 'zhipaiquyangren', text: '指派的取样人名称，获取当前登录用户外键T_S_Base_User表的UserName字段' })
      fieldList.push({
        type: 'string',
        value: 'zhipaiquyangren',
        text: '指派的取样人名称，获取当前登录用户外键T_S_Base_User表的UserName字段',
        dictCode: ''
      })
      fieldList.push({ type: 'string', value: 'zhipaishijian', text: '指派时间（YYYY-MM-dd HH:mm:ss）' })
      fieldList.push({ type: 'string', value: 'zhipaishijian', text: '指派时间（YYYY-MM-dd HH:mm:ss）', dictCode: '' })
      fieldList.push({ type: 'string', value: 'zhipairen', text: '指派人，获取当前登录用户外键T_S_Base_User表的UserName字段' })
      fieldList.push({
        type: 'string',
        value: 'zhipairen',
        text: '指派人，获取当前登录用户外键T_S_Base_User表的UserName字段',
        dictCode: ''
      })
      fieldList.push({ type: 'int', value: 'zhipaizhuangtai', text: '指派状态（0：未指派1：已指派）', dictCode: '' })
      fieldList.push({ type: 'int', value: 'zhipaizhuangtai', text: '指派状态（0：未指派1：已指派）' })
      fieldList.push({
        type: 'string',
        value: 'zhipaiquyangrenid',
        text: '指派的取样人id，获取当前登录用户外键T_S_Base_User表的id字段',
        dictCode: ''
      })
      ...
      fieldList.push({ type: 'string', value: 'fujian', text: 'fujian', dictCode: '' })
