...
<!--      <a-button type="primary" icon="download" @click="handleExportXls('孔道灌浆（定性检测）主表')">导出</a-button>-->
...
<!--      </a-upload>-->
    ...
    <tr-kongdaogj-dxm-modal ref="modalForm" @ok="modalFormOk"/>
  </a-card>
</template>
...
<script>
  ...
  import TrKongdaogjDxmModal from './modules/TrKongdaogjDxmModal'
  import '@/assets/less/TableExpand.less'

  export default {
    name: "TrKongdaogjDxmList",
    ...
    data () {
      return {
        ...
        url: {
          list: "/trkongdaogjdxm/trKongdaogjDxm/list",
          delete: "/trkongdaogjdxm/trKongdaogjDxm/delete",
          deleteBatch: "/trkongdaogjdxm/trKongdaogjDxm/deleteBatch",
          exportXlsUrl: "/trkongdaogjdxm/trKongdaogjDxm/exportXls",
          importExcelUrl: "trkongdaogjdxm/trKongdaogjDxm/importExcel",
    ...
    methods: {
      ...
      getSuperFieldList(){
        let fieldList=[];
         ...
         fieldList.push({type:'BigDecimal',value:'holede',text:'孔道高差(单位：m)',dictCode:''})
         fieldList.push({type:'number',value:'holede',text:'孔道高差(单位：m)'})
         fieldList.push({type:'int',value:'anchorropev',text:'锚索波速(m/s)',dictCode:''})
         fieldList.push({type:'int',value:'anchorropev',text:'锚索波速(m/s)'})
         fieldList.push({type:'number',value:'ch1length',text:'传感器 1 到梁板长度(单位m)'})
         fieldList.push({type:'BigDecimal',value:'ch1length',text:'传感器 1 到梁板长度(单位m)',dictCode:''})
         fieldList.push({type:'number',value:'ch2length',text:'传感器 2 到梁板长度(单位m)'})
         fieldList.push({type:'BigDecimal',value:'ch2length',text:'传感器 2 到梁板长度(单位m)',dictCode:''})
