        ...
        <a-row :gutter="24">
          ...
          <a-col :xl="6" :lg="6" :md="6" :sm="8">
            <a-form-item label="部位名称">
              <a-input placeholder="请输入部位名称" v-model="queryParam.conspos1" @change="renwudan"></a-input>
            </a-form-item>
          </a-col>
          ...
          <a-col :xl="4" :lg="6" :md="6" :sm="10">
            <a-form-item label="时间:">
              <j-date placeholder="开始" v-model="queryParam.dattim_begin" :showTime="true"
                      dateFormat="YYYY-MM-DD HH:mm:ss"/>
            </a-form-item>
          </a-col>
          <a-col :xl="4" :lg="6" :md="6" :sm="8">
            <a-form-item label="">
              <j-date placeholder="结束" v-model="queryParam.dattim_end" :showTime="true"
                      dateFormat="YYYY-MM-DD HH:mm:ss"/>
            </a-form-item>
          </a-col>
          <a-col :xl="4" :lg="7" :md="8" :sm="8">
            <a-form-item label="质检">
              <j-dict-select-tag placeholder="请选择" v-model="queryParam.iszjzl"
                                 dictCode="iszjzl"></j-dict-select-tag>
            </a-form-item>
          </a-col>
<!--          <a-col :xl="4" :lg="6" :md="8" :sm="8">-->
<!--            <a-form-item label="审核" >-->
<!--              <a-checkbox  @change = "isjz('2')" > 未审核</a-checkbox>-->
<!--            </a-form-item>-->
<!--          </a-col>-->
                    <a-col :xl="4" :lg="6" :md="8" :sm="8">
                      <a-form-item label="是否审核">
                        <j-dict-select-tag placeholder="请选择" v-model="queryParam.checkStatus"
                                           dictCode="checkStatus"></j-dict-select-tag>
                      </a-form-item>
                    </a-col>
    ...
    <div class="table-operator">
      <a-button @click="handleAdd47" type="primary" v-has="'shigongphb:addjzl'" icon="plus">新增浇筑令配料</a-button>
      <a-button @click="handleAdd" type="primary" v-has="'shigongphb:add'" icon="plus">新增配料单</a-button>
      <a-button @click="handleAdd1" type="primary" v-has="'phb:hege'" icon="plus">新增配料单</a-button>
      ...
      <a-upload name="file" v-has="'shigongphb:dr'" :showUploadList="false" :multiple="false" :headers="tokenHeader"
    ...
    <div>
      ...
      >
         ...
         <span slot="iszjzl" slot-scope="iszjzl, record">
          ...
          <a-tag color="green" v-if="record.iszjzl==1">待确认</a-tag>
          <a-tag color="green" v-if="record.iszjzl==2">推送成功</a-tag>
          <a-tag color="red" v-if="record.iszjzl==3">推送失败</a-tag>
          <a-tag color="yellow" v-if="record.iszjzl==4">未关联浇筑令</a-tag>
        </span>
...
<!--        <a-tag v-if="record.checkStatus == 0" color="green">{{ record.code }}</a-tag>-->
<!--          <a-tag v-else color="red">审核后显示</a-tag>-->
<!--       </span>-->
        <span slot="code" slot-scope="code, record">
        ...
        <span slot="action" slot-scope="text, record">
          <a v-has="'shigongphb:edit'" @click="handleEdit(record)">编辑</a>
           <a-divider v-has="'shigongphb:edit'" type="vertical"/>
             <a v-has="'shigongphb:mount'" @click="mountData(record)">生产填报</a>
          <a-divider v-has="'shigongphb:mount'" type="vertical"/>
          <a-dropdown>
            <a class="ant-dropdown-link">更多 <a-icon type="down"/></a>
            <a-menu slot="overlay">
<!--              <a-menu-item v-if="record.checkStatus===null || record.checkStatus===3" v-has="'shigongphb:sh'">-->
...
<!--              </a-menu-item>-->
    ...
    <shigongphb-modal  ref="modalForm" @ok="modalFormOk"></shigongphb-modal>
    <shigongphb-modal47  ref="modalForm47" @ok="modalFormOk"></shigongphb-modal47>
    <ShigongphbModal1 v-has="'phb:hege'"  ref="modalForm1" @ok="modalFormOk"></ShigongphbModal1>
    <mount-data-modal ref="mountData"></mount-data-modal>
    <shigongphb-shen-he ref="shigongphbSH" @change="change"></shigongphb-shen-he>
  </a-card>
</template>
...
import ShigongphbModal1 from './modules/ShigongphbModal1'
import ShigongphbModal47 from './modules/ShigongphbModal47'
import ShigongphbModal from './modules/ShigongphbModal'
import JSuperQuery from '@/components/jeecg/JSuperQuery.vue'
import { SYS_DEPART_ORGCODE } from '@/store/mutation-types'
import { usershebeiList } from '@api/api'
import Vue from 'vue'
import MountDataModal from './modules/MountDataModal'
import ShigongphbShenHe from './ShigongphbShenHe'
  ...
  data() {
    return {
      ...
      url: {
        ...
        printUrl: 'jmreport/view/967652258288873472',
        resetPush: '/system/shigongphb/resetPush'
  ...
  methods: {
    isjz(a) {
      ...
      if(a === '2'){
       ...
       // this.queryParam.checkStatus_end = 0
        this.queryParam.superQueryMatchType= 'and'
        this.queryParam.superQueryParams = "%5B%7B%22rule%22:%22ne%22,%22type%22:%22int%22,%22val%22:0,%22field%22:%22checkStatus%22%7D,%7B%22rule%22:%22ne%22,%22type%22:%22int%22,%22val%22:1,%22field%22:%22checkStatus%22%7D%5D"
      }else{
        this.queryParam.superQueryMatchType= ''
        this.queryParam.superQueryParams = ""

      }
    },
    handleAdd47: function () {
      this.$refs.modalForm47.add();
      this.$refs.modalForm47.title = "新增";
      this.$refs.modalForm47.disableSubmit = false;
    },
    handleAdd1: function () {
    ...
    handleShenhe: function (data) {
      this.$refs.shigongphbSH.modelObj = Object.assign({}, data)
      this.$refs.shigongphbSH.visible1 = true
    },
    renwudan() {
    ...
    mountData(record) {
      this.$refs.mountData.modelData = Object.assign({}, record)
