          ...
          <a-row>
            ...
            <a-col :span="12">
              <a-form-item label="计划方量" :labelCol="labelCol" :wrapperCol="wrapperCol">
                <a-input v-decorator="['mete',validatorRules.mete]" placeholder="请输入计划方量" :disabled="formDisabled"></a-input>
              </a-form-item>
            </a-col>
          </a-row>
          <a-row>
...
<!--              </a-form-item>-->
              <a-form-item v-if="!formDisabled" label="选择施工部位" :labelCol="labelCol" :wrapperCol="wrapperCol">
                <JSelectDqProjName v-decorator="['zhiliangcode',validatorRules.zhiliangcode]" ::multi="false"/>
              </a-form-item>
            </a-col>
          </a-row>
          <a-row>
            ...
            <a-col :span="12">
              <a-form-item label="浇筑日期" :labelCol="labelCol" :wrapperCol="wrapperCol">
                <j-date placeholder="请选择浇筑日期" :showTime="showTime"
                        v-decorator="['jztime', { rules: [{ required: true, message: '请选择浇筑日期！' }] }]"
                        :trigger-change="true"
...
<!--    <bhzrenwudan-modal ref="rwdadd" @ok="modalFormOk"></bhzrenwudan-modal>-->
  </a-spin>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      validatorRules: {
        zhiliangcode: {
          rules: [{ validator: this.zhiliangcode }]
        },
        mete: {
          rules: [
            {
        ...
        shebeibianhao: {
          rules: [{ validator: this.shebeibianhao }]
        },
        renwudan: {
          rules: [
            {
        ...
        pici:{
          rules: [{ validator: this.validatepici1 }],
        },
        pici3:{
      ...
      coo:0,
      url: {
        add: "/sys/sysDepartproject47/add1",
  ...
  methods: {
    // handleAdd: function () {
    //   this.$refs.rwdadd.add();
    //   this.$refs.rwdadd.title = "新增";
    //   this.$refs.rwdadd.disableSubmit = false;
    // },
    add() {
      this.edit({});
    },
    ...
    addrenwuControl (shebeino) {
      getAction(this.url.controls,{shebeiNo : shebeino,id_begin: 8152691}).then((res) => {
        if (res.success) {
          console.log(res.result)
          if(res.result.coo>0) {
            this.coo = res.result.coo
            // callback("该拌合站存在未使用浇筑令的生产数据，请及时填报原因进行审批后可生产")
              this.$message.error("该拌合站存在未使用浇筑令的生产数据，请及时填报原因进行审批后可生产")
    ...
    handleChangeshebei(selectedValue) {
      ...
      this.addrenwuControl(selectedValue)
    ...
    zhiliangcode(rule, value, callback) {
      console.log("选择的部位值：", value)

      if (!value) {
        callback(new Error('请选择部位!')) // 使用 Error 对象是更标准的做法
        return // 直接返回，不再继续执行
      }

      getAction(this.url.aaa, { id: value }).then(res1 => {
        console.log("res1.result", res1.result)
        if (res1.result != 1) {
          callback(new Error('请选择最底层部位！'))
        } else {
          callback() // 只有验证通过时才调用无参 callback
        }
      }).catch(error => {
    ...
    shebeibianhao(rule, value, callback) {
      // 获取表单
      const form = this.form
      // 获取要校验的批次字段
      const str =rule.field
      ...
      getAction(this.url.list2, params).then((res) => {
        ...
        if (res.success) {
          ...
          for (let i = 0; i < list1.length; i++) {
            if (list1[i].materialTypes == 6) {
            ...
            } else if (list1[i].materialTypes == 1) {
              console.log(list1[i], "+++++++++++++++++++++++++++");
              this.model.m4 = list1[i].materialname;
              this.model.u4 = list1[i].amount;
              this.model.ru4 = list1[i].amount;
            } else if (
              ...
              switch (this.dataSource.length) {
                case 1:
                  this.model.m5 = this.dataSource[0].materialname;
                  this.model.u5 = this.dataSource[0].amount;
                  this.model.ru5 = this.dataSource[0].amount;
                  break;
                case 2:
                  this.model.m5 = this.dataSource[0].materialname;
                  this.model.u5 = this.dataSource[0].amount;
                  this.model.ru5 = this.dataSource[0].amount;
                  this.model.m6 = this.dataSource[1].materialname;
                  this.model.u6 = this.dataSource[1].amount;
                  this.model.ru6 = this.dataSource[1].amount;
                  break;
                case 3:
                  this.model.m5 = this.dataSource[0].materialname;
                  this.model.u5 = this.dataSource[0].amount;
                  ...
                  this.model.u6 = this.dataSource[1].amount;
                  this.model.ru6 = this.dataSource[1].amount;
                  this.model.m7 = this.dataSource[2].materialname;
                  this.model.u7 = this.dataSource[2].amount;
                  this.model.ru7 = this.dataSource[2].amount;
                  break;
                case 4:
                  this.model.m5 = this.dataSource[0].materialname;
                  this.model.u5 = this.dataSource[0].amount;
                  ...
                  this.model.u7 = this.dataSource[2].amount;
                  this.model.ru7 = this.dataSource[2].amount;
                  this.model.m12 = this.dataSource[3].materialname;
                  this.model.u12 = this.dataSource[3].amount;
                  this.model.ru12 = this.dataSource[3].amount;
                  break;
                // case 5:
