    ...
    <a-spin :spinning="confirmLoading">
      <a-form-model ref="form" :model="model" :rules="validatorRules">
        ...
        <a-form-model-item
          ...
          prop="menuPerms"
          label="菜单路径">
          <a-input placeholder="请输入菜单路径" v-model="model.menuPerms" :readOnly="disableSubmit"/>
        </a-form-model-item>
...
<!--        </a-form-model-item>-->
        ...
        <a-form-model-item
          ...
          prop="btnPerms"
          label="授权标识">
          <a-input placeholder="请输入授权标识, 如: user:list" v-model="model.btnPerms" :readOnly="disableSubmit"/>
        </a-form-model-item>
        ...
        <a-form-item
          ...
          label="上传文件/图片" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <j-image-upload v-model="model.imgurl" ></j-image-upload>
        </a-form-item>
...
<!--        </a-form-model-item>-->
        ...
        <a-form-model-item
          ...
          :wrapperCol="wrapperCol"
          label="app标签">
          <a-switch checkedChildren="是" unCheckedChildren="否" v-model="model.isLabel"/>
        </a-form-model-item>
...
<script>
  import {addAppPermission,editAppPermission,queryTreeAppList, duplicateCheck} from '@/api/api'
  import Icons from './icon/Icons'
  import JImageUpload from "@comp/jeecg/JImageUpload";

  export default {
    name: "AppPermissionModal",
    components: {Icons,JImageUpload},
    data () {
      return {
        drawerWidth:700,
        treeData:[],
    ...
    computed:{
      validatorRules:function() {
        return {
          ...
          btnPerms:[{ required: false, message: '请输入授权标识!' },{validator: this.validatebtnPerms }],
          menuPerms:[{ required: false, message: '请输入菜单路径!' },{validator: this.validatemenuPerms }]
        }
      }
    },
    created () {
    ...
    methods: {
      ...
      edit (record) {
        ...
        // this.shows = record.menuType==2?false:true;
      ...
      handleOk () {
        ...
        this.$refs.form.validate(valid => {
          if (valid) {
            ...
            let obj;
            if (!this.model.id) {
              obj = addAppPermission(this.model);
            } else {
              obj = editAppPermission(this.model);
            }
            obj.then((res) => {
              if (res.success) {
      ...
      validatebtnPerms(rule, value, callback){
      ...
      validatemenuPerms(rule, value, callback){
        if(value && value.length>0){
          //校验授权标识是否存在
          var params = {
            tableName: 'sys_app_permission',
            fieldName: 'menu_perms',
            fieldVal: value,
            dataId: this.model.id
          };
          duplicateCheck(params).then((res) => {
