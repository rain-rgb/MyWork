...
export default {
  ...
  methods: {
    ...
    getPiciResidu2(pici) {
      ...
      getAction(this.url.getpicire, params).then((res) => {
        console.log("getPiciResidu",res)
        if (res.success) {
          // 获取该批次剩余量
         var a = (res.result.records[0].pizhongt)
    ...
    getPiciResidu(field) {
      ...
      // 获取余量字段
      var residuename = 'piciRS'+field.substring(4,field.length)
      //  余料是否有值
      var residue = form.getFieldValue(residuename)
      // 如果设置了料仓，则需要设置检验批次
      if(!residue){
        residue = 0
      }
    ...
    validatepici1(rule, value, callback) {
      ...
      if (!value) {
       ...
       //  var residue = 0
        ...
        // this.$nextTick(()=>{
        //   let params = {
        //     pici: form.getFieldValue(str),
        //   };
        //   getAction(this.url.getpicire, params).then((res) => {
        //     if (res.success) {
        //       // 获取该批次剩余量
        //       var a = (res.result.records[0].pizhongt)
        //       residue = (a.substring(0,a.length-1)*1000).toFixed(2)
        //       // var residue = 100
        //        can1 = (residue/uses).toFixed(2)
        //       // 如果余量小于批次用量则进行提醒
        //       if((residue - uses)<0){
