    ...
    <j-form-container :disabled="formDisabled">
      <a-form-model ref="form" :model="model" :rules="validatorRules" slot="detail">
        <a-row v-if="process === 0 || process === 3 ">
          ...
          <!--          </a-col>-->
          <a-col :span="24">
            <a-form-model-item label="问题原因" :labelCol="labelCol" :wrapperCol="wrapperCol"  :rules="[{ required: true, message: '问题原因不能为空' }]" prop="problemReasons">
              <a-input v-model="model.problemReasons" placeholder="请输入问题原因" :disabled="chuzhidis" ></a-input>
            </a-form-model-item>
          </a-col>
          <a-col :span="24">
            <a-form-model-item label="处理方式" :labelCol="labelCol" :wrapperCol="wrapperCol"  :rules="[{ required: true, message: '处理方式不能为空' }]" prop="handleWay">
              <a-input v-model="model.handleWay" placeholder="请输入处理方式" :disabled="chuzhidis" ></a-input>
            </a-form-model-item>
          </a-col>
          <a-col :span="24">
            <a-form-model-item label="处理结果" :labelCol="labelCol" :wrapperCol="wrapperCol" :rules="[{ required: true, message: '处理结果不能为空' }]" prop="handleResult">
...
<script>
  ...
  export default {
    name: 'PippileOneOverHandlerForm',
    components: {
      JImageUpload
    },
    watch:{
    ...
    data () {
      return {
        ...
        url: {
          add: "/pippileOneOverHandler/pippileOneOverHandler/handle",
          edit: "/pippileOneOverHandler/pippileOneOverHandler/edit",
          queryById: "/pippileOneOverHandler/pippileOneOverHandler/queryById"
        }
      }
    },
    computed: {
