        ...
        <a-row :gutter="24">
          ...
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="桩号">
              <a-input placeholder="请输入桩号" v-model="queryParam.pileNo"></a-input>
            </a-form-item>
          </a-col>
        </a-row>
        <a-row>
...
<!--      <a-button type="primary" icon="download" v-has="'mixpilehistoryone:dcs'" @click="handleExportXls2('搅拌桩成桩记录信息表')">单个搅拌桩成桩记录表导出</a-button>-->
      <a-button type="primary" icon="download" v-has="'mixpilehistoryone:dc'" @click="handleExportXls2('搅拌桩成桩记录信息表')">导出</a-button>
      <a-upload name="file"  :showUploadList="false" :multiple="false" :headers="tokenHeader" :action="importExcelUrl" @change="handleImportExcel">
        <a-button type="primary" v-has="'mixpilehistoryone:dr'" icon="import">导入</a-button>
      </a-upload>
      <a-button type="primary" icon="download" @click="print1">申浙导出</a-button>
      <!-- 高级查询区域 -->
...
<!--        <span  slot="pileDownbeton" slot-scope="pileDownbeton, record" v-if="record.pileDownbeton!==null"> {{ record.pileDownbeton.substring(0,record.pileDownbeton.indexOf(".")+4) }}</span>-->
        <span  slot="pileCement" slot-scope="pileCement, record" v-if="record.pileCement!==null"> {{ record.pileCement.substring(0,record.pileCement.indexOf(".")+5) }}</span>
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          list: "/deviceMixpileHistorydataOne/deviceMixpileHistorydataOne/list",
          ...
          printUrl: "jmreport/view/932188094930202624",
    ...
    methods: {
      ...
      print1: function () {//打印功能需要先去报表设计页面设计打印格式
        ...
        let url = `${window._CONFIG['domianURL']}/${this.url.printUrl}?shebeino=${param.shebeino}&start=${param.pileTime_begin}&endtime=${param.pileTime_end}&token=${this.token}`
        window.open(url)
      },
