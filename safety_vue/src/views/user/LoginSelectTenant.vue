...
<script>
  ...
  import { USER_INFO,SYS_DEPART_ORGCODE } from "@/store/mutation-types"
          ...
          if(this.isMultiDepart===false){
            ...
            putAction("/sys/selectDepart",obj).then(res=>{
              if(res.success){
                ...
                Vue.ls.set(SYS_DEPART_ORGCODE, userInfo.orgCode, 7 * 24 * 60 * 60 * 1000)
                this.$store.commit('SET_INFO', userInfo);
                this.$store.commit('SYS_DEPART_ORGCODE', userInfo.orgCode);
