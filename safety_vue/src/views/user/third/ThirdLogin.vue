...
<!--    </div>-->
    <!--Android APP 二维码信息-->
    <div class="user-login-other" style="display: flex;">
      <span style="color: #f0f2f5">APP</span>
      <a @mouseover="shows()"  @mouseout="shows1()" title=""><a-icon :style="{ color: 'white' }" class="item-icon" type="android" /></a>
        <img v-if="APPsrcshow" :src="APPsrc" style="margin-left: 25px;height: 160px;width: 160px"/>
    </div>
...
import {selectAPPmessage} from '@/api/api'
...
export default {
  ...
  created() {
    this.selectAPPmessage();
  },
  methods:{
    selectAPPmessage(){//展示APP二维码信息
      let param={type:1}
      selectAPPmessage(param).then(res=>{
        this.APPsrc=res.result.srcs;
      })
    },

    shows(){
      setTimeout(this.APPsrcshow=true,1000)
    },
    shows1(){
      setTimeout(this.APPsrcshow=false,1000)
    }
  }
}
</script>
