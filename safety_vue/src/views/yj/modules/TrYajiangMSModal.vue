    ...
    <a-tabs default-active-key="1" @change="callback2">
      <a-tab-pane key="1" tab="智能压浆过程数据">
        <a-table rowKey="sid" :columns="columns1" :data-source="data1" :scroll="{ y: 400 }" :pagination="false">
          <span slot="zt" slot-scope="zt, record">
            <a-tag v-if="record.zt === '1'">开始压浆</a-tag>
            <a-tag v-if="record.zt === '2'">保压</a-tag>
            <a-tag v-if="record.zt === '3'">倒计时</a-tag>
            <a-tag v-if="record.zt === '4'">倒计时结束</a-tag>
            <a-tag v-if="record.zt === '5'">压浆完毕</a-tag>
          </span>
        </a-table>
      </a-tab-pane>
