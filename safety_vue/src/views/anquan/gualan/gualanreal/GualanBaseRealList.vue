<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      <a-button @click="handleAdd" type="primary" v-has="'gualanreal:add'" icon="plus">新增</a-button>
      <a-button type="primary" icon="download" v-has="'gualanreal:dc'" @click="handleExportXls('挂篮实时数据表')">导出</a-button>
      <a-upload name="file" :showUploadList="false" :multiple="false" :headers="tokenHeader" :action="importExcelUrl" @change="handleImportExcel">
        <a-button type="primary" v-has="'gualanreal:dr'" icon="import">导入</a-button>
      </a-upload>
      <!-- 高级查询区域 -->
...
<!--      </a-dropdown>-->
    ...
    <div>
      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          <a v-has="'gualanreal:edit'" @click="handleEdit(record)">编辑</a>
          ...
          <a-dropdown>
            ...
            <a-menu slot="overlay">
              ...
              <a-menu-item v-has="'gualanreal:del'" >
    ...
    <gualan-base-real-modal ref="modalForm" @ok="modalFormOk"></gualan-base-real-modal>
...
<script>
  ...
  import GualanBaseRealModal from './modules/GualanBaseRealModal'
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          list: "/gualanreal/gualanBaseReal/list",
          delete: "/gualanreal/gualanBaseReal/delete",
          deleteBatch: "/gualanreal/gualanBaseReal/deleteBatch",
          exportXlsUrl: "/gualanreal/gualanBaseReal/exportXls",
          importExcelUrl: "gualanreal/gualanBaseReal/importExcel",
    ...
    methods: {
      ...
      getSuperFieldList(){
        ...
        fieldList.push({type:'double',value:'tension1',text:'1#螺纹钢拉力(放大100倍单位T)(实际显示为30.00T)',dictCode:''})
        fieldList.push({type:'double',value:'tension2',text:'2#螺纹钢拉力(放大100倍单位T)(实际显示为30.00T)',dictCode:''})
        fieldList.push({type:'double',value:'tension3',text:'3#螺纹钢拉力(放大100倍单位T)(实际显示为30.00T)',dictCode:''})
        fieldList.push({type:'double',value:'tension4',text:'4#螺纹钢拉力(放大100倍单位T)(实际显示为30.00T)',dictCode:''})
        fieldList.push({type:'double',value:'levelness1',text:'下支撑横梁水平度（放大10倍单位度）(实际显示为1.0度)',dictCode:''})
        fieldList.push({type:'double',value:'levelness2',text:'下支撑横梁水平度（放大10倍单位度）(实际显示为1.0度)',dictCode:''})
        ...
        fieldList.push({type:'double',value:'basket',text:'挂篮前端下沉量（放大10倍单位mm）',dictCode:''})
        fieldList.push({type:'double',value:'windspeed',text:'风速（放大10倍 单位m/s）',dictCode:''})
        fieldList.push({type:'int',value:'windspeedwarn',text:'风速报警0---正常  1---报警',dictCode:''})
        fieldList.push({type:'double',value:'reservedOne',text:'下托梁有效长度',dictCode:''})
      ...
      handle(){
        if(this.selectionRows.length){
          ...
          this.selectionRows.forEach(item => {
            if(item.shebeibanhao != this.selectionRows[0].shebeibanhao){
              type = false
            }
          })
          if(!type){
            ...
            this.chuzhiId = strID
            this.shebeiNo = this.selectionRows[0].shebeibanhao
