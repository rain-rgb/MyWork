...
export default {
  ...
  data() {
    ...
    return {
      ...
      validatorRules: {},
      url: {
        list: '/clgl/frontDeviceHistorydata/listData',
...
//初始化地图
    ...
    Query() {//搜索
      ...
      let params = { shebeiNo: this.shebeiNo, datatime_end: this.datatime_end, datatime_begin: this.datatime_begin }
      getAction(this.url.list, params).then((res) => {
        if (res.success) {
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            if (i <= data.length - 1) {
              this.center.push(data[0].longitude, data[0].latitude)
            }
          }
    ...
    Carguiji: function (record) {
      ...
      let params = { shebeiNo: this.shebeiNo, datatime_end: record.datatime, datatime_begin: datatime_start }
      getAction(this.url.list, params).then((res) => {
        if (res.success) {
          if (res.result.length > 0) {
            let data = res.result
