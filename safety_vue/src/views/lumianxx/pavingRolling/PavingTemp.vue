...
export default {
  ...
  methods: {
    ...
    getChartOne() {
      ...
      this.$http1.post(`/api/pavement/paveTempPass.shtml`, param).then((res) => {
        if (res.code === 0) {
          let arr = res.data;
          this.getecharts(arr);
          // this.getecharts1(arr);
          console.log(res, "getChartOne 摊铺温度统计------------------------");
        }
        this.loading = false;
      });
    },
    ...
    getecharts(data) {
      ...
      let chart = echarts.init(document.getElementById("PavingTemp"));
