      ...
      <a-row>
        ...
        >
          ...
          <div class="charts-box">
            <div id="rollingcharts"></div>
            <div id="rollingcharts1"></div>
          </div>
        </a-card>
      </a-row>
    </div>
  ...
  methods: {
    ...
    getChartOne() {
      let param = {
        ...
        sectionId: "jishu01@20210801184014",
        startTime: this.form.startTime,
        endTime: this.form.endTime,
      };
      this.$http1.post(`/api/pavement/rollingSpeedPass.shtml`, param).then((res) => {
        if (res.code == 0) {
          let arr = res.data;
          this.getecharts(arr);
          this.getecharts1(arr);
          console.log(res, "getChartOne 碾压速度统计------------------------");
        }
        this.loading = false;
      });
    },
    getecharts(data) {
      ...
      let chart = echarts.init(document.getElementById("rollingcharts"));
    ...
    getecharts1(data) {
      ...
      let chart = echarts.init(document.getElementById("rollingcharts1"));
      chart.setOption(option);
      //建议加上以下这一行代码，不加的效果图如下（当浏览器窗口缩小的时候）。超过了div的界限（红色边框）
      window.addEventListener("resize", function () {
        chart.resize();
