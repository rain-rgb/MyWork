...
export default {
  ...
  methods: {
    ...
    getChartOne() {
      ...
      this.$http1.post(`/api/pavement/maxTempPass.shtml`, param).then((res) => {
        if (res.code == 0) {
          let arr = res.data;
          this.getecharts(arr);
          this.getecharts1(arr);
          console.log(res, "getChartOne 最高温度总体统计------------------------");
        }
        this.loading = false;
      });
    },
    ...
    getChartThree(){
      ...
      this.$http1.post(`/api/pavement/stationData.shtml`, param).then((res) => {
        if (res.code == 0) {
          ...
          this.getcharts(arr);
          // this.getecharts1(arr);
          console.log(res, "getChartOne 最高温度分布------------------------");
        }
        this.loading = false;
      });
    },
    ...
    getecharts(data) {
      ...
      arr[0].value = data.steel;
      arr[1].value = (1 - data.steel).toFixed(2);
      let option = {
        color: colorList,
        title: {
          text: "钢轮温度",
    ...
    getecharts1(data) {
      ...
      arr[0].value = data.rubber;
      arr[1].value = (1 - data.rubber).toFixed(2)
      let option = {
        color: colorList,
        title: {
          text: "胶轮温度",
    ...
    getcharts(data) {
      ...
      data.forEach((e)=>{
        //左右幅，0为左幅，1为右幅
        if(e.offset == 1){
          arr.push(e.temperatureFirst)
        }else{
          arr1.push(e.temperatureFirst)
        }
      })
      ...
      let dailyCharts = null;
      // if (dailyCharts) {
      //   dailyCharts.dispose();
      // }
      dailyCharts = echarts.init(document.getElementById("Tempcharts"));
    ...
    getcharts1() {
      ...
      let dailyCharts = null;
      dailyCharts = echarts.init(document.getElementById("Tempcharts1"));
      dailyCharts.setOption(option);
    },
  },
};
